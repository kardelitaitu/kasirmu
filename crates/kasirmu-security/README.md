...
`kasirmu-security` owns TLS configuration (`tls`), sensitive-data masking including the
...
`kasirmu-crypto` crate, applied by the typed accessors in
...
settings columns are encoded by `kasirmu-crypto` under a derived key. They are not
