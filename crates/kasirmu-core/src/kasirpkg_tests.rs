...
fn roundtrip_minimal() {
    ...
    let (header, imported) = import_kasirpkg(&exported, "test-password-123").unwrap();
...
fn roundtrip_with_data() {
    ...
    let (_header, imported) = import_kasirpkg(&exported, "strong-password-here!").unwrap();
...
fn wrong_password_fails() {
    ...
    let result = import_kasirpkg(&exported, "wrong-password");
    assert!(
        result.is_err(),
...
fn corrupted_data_fails() {
    ...
    let mut exported = export_kasirpkg(
        "password",
        "Store",
        "0.0.1",
        vec![],
    ...
    let result = import_kasirpkg(&exported, "password");
    assert!(
        result.is_err(),
...
fn empty_password_is_refused_at_export() {
    ...
    // export_kasirpkg("") succeeded "(though not recommended)". B50 reversed
    ...
    let err = export_kasirpkg("", "Store", "0.0.1", vec![], HashMap::new(), &payload)
...
fn header_metadata_preserved() {
    ...
    let (header, _) = import_kasirpkg(&exported, "password").unwrap();
...
fn large_payload_roundtrip() {
    ...
    let (_header, imported) = import_kasirpkg(&exported, "large-payload-password").unwrap();
...
fn empty_payload() -> KasirpkgPayload {
    KasirpkgPayload {
        products: vec![],
        categories: vec![],
        sales: None,
...
fn tampering_with_the_header_store_name_is_detected() {
    ...
    import_kasirpkg(&tampered, "password")
...
fn build_v1_archive(password: &str, store_name: &str, payload: &KasirpkgPayload) -> Vec<u8> {
...
// ── import_kasirpkg error paths ─────────────────────────────────────────────────

#[test]
fn import_too_short_data_fails() {
    let result = import_kasirpkg(&[0u8; 10], "password");
...
fn import_empty_data_fails() {
    let result = import_kasirpkg(&[], "password");
    assert!(result.is_err());
}
...
fn import_exactly_header_len_with_invalid_json_fails() {
    ...
    let result = import_kasirpkg(&data, "password");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
...
#[test]
fn kasirpkg_header_serde_roundtrip() {
    let header = KasirpkgHeader {
        version: 2,
    ...
    let parsed: KasirpkgHeader = serde_json::from_str(&json).unwrap();
...
#[test]
fn kasirpkg_payload_serde_roundtrip_with_all_fields() {
    let payload = KasirpkgPayload {
    ...
    let parsed: KasirpkgPayload = serde_json::from_str(&json).unwrap();
...
#[test]
fn kasirpkg_payload_serde_skips_none_fields() {
    let payload = KasirpkgPayload {
        products: vec![],
        categories: vec![],
        sales: None,
...
#[test]
fn kasirpkg_header_debug_and_clone() {
    let header = KasirpkgHeader {
        version: 2,
    ...
    assert!(debug.contains("KasirpkgHeader"));
    assert!(debug.contains("Store"));
}

#[test]
fn kasirpkg_payload_debug_and_clone() {
    let payload = empty_payload();
    let cloned = payload.clone();
    assert!(cloned.products.is_empty());

    let debug = format!("{payload:?}");
    assert!(debug.contains("KasirpkgPayload"));
}
...
fn unicode_in_payload_roundtrip() {
    ...
    let (_, imported) = import_kasirpkg(&exported, "password").unwrap();
...
fn each_export_uses_unique_salt_and_nonce() {
    ...
    let (header_a, _) = import_kasirpkg(&a, "password").unwrap();
    let (header_b, _) = import_kasirpkg(&b, "password").unwrap();
