...
// ── log_audit: the transaction states (C8-adjacent audit recipe) ─────

/*
 * HONESTY NOTE, part of the acceptance: NONE of these four tests can
 * discriminate pre-fix from post-fix on its own. The bare
 * `self.conn.execute(...)` this replaced was already atomic in autocommit and
 * already JOINED an open transaction, so it passes all four unchanged. The
 * discriminating test is `log_audit_inside_open_tx_does_not_error` run against
 * the NAIVE WRAP (an unconditional `unchecked_transaction()`), which fails with
 * "cannot start a transaction within a transaction" — that is what the branch
 * buys, and it is why the branch exists.
 */

/// One audit entry keyed by `target_id` (the id is generated by `new`).
fn entry_for(target: &str) -> AuditEntry {
    AuditEntry::new(
        "user-1",
        "sale.create",
        Some("sale".to_string()),
...
#[test]
fn log_audit_standalone_writes_exactly_one_committed_row() {
    let conn = fresh();
    let s = store(&conn);
    s.log_audit(&entry_for("sale-standalone")).unwrap();
    assert_eq!(audit_count(&conn), 1, "one call, one row");
    assert!(
        conn.is_autocommit(),
        "log_audit must not leave a transaction open on the caller's connection: the next write on this handle would silently join it"
    );
}

#[test]
fn log_audit_inside_open_tx_does_not_error() {
    let mut conn = fresh();
    let tx = conn.transaction().unwrap();
    let s = Store::new(&tx);
    let result = s.log_audit(&entry_for("sale-in-tx"));
    assert!(
        result.is_ok(),
        "log_audit must JOIN the caller's transaction, never open a nested BEGIN — SQLite has no nested BEGIN, so an unconditional unchecked_transaction() fails with \"cannot start a transaction within a transaction\"; got {:?}",
        result.err()
    );
    // The row is visible INSIDE the transaction, from the same connection.
    assert_eq!(audit_count(&tx), 1, "the row joins the open transaction");
    drop(tx); // rollback
    assert_eq!(
        audit_count(&conn),
        0,
        "the joined row dies with the transaction it joined"
    );
}

#[test]
fn log_audit_inside_open_tx_is_lost_on_caller_rollback() {
    // The deliberate semantic, stated as a test: an action whose transaction
    // rolled back did NOT happen, so an audit row for it would be a phantom —
    // a record of an event that never occurred. A caller that needs a row to
    // survive its own rollback writes on its own connection after committing.
    let mut conn = fresh();
    // Short-lived Store handles: `conn.transaction()` needs the mutable
    // borrow, so no Store may stay alive across it.
    store(&conn)
        .log_audit(&entry_for("sale-committed"))
        .unwrap();
    assert_eq!(audit_count(&conn), 1);

    let tx = conn.transaction().unwrap();
    Store::new(&tx)
        .log_audit(&entry_for("sale-rolled-back"))
        .unwrap();
    assert_eq!(audit_count(&tx), 2, "both rows are visible inside the tx");
    tx.rollback().unwrap();

    assert_eq!(
        audit_count(&conn),
        1,
        "the rolled-back action leaves no audit trail, and the committed row is untouched"
    );
    let rows = store(&conn).list_audit_entries(10, 0).unwrap();
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].target_id.as_deref(), Some("sale-committed"));
}

#[test]
fn log_audit_never_writes_two_rows_for_one_call() {
    let conn = fresh();
    let s = store(&conn);
    s.log_audit(&entry_for("sale-once")).unwrap();
    assert_eq!(audit_count(&conn), 1, "one call must write exactly one row");
    // A second, SEPARATE call is a second row — the assertion above is about
    // one call, not about a de-duplicating writer.
