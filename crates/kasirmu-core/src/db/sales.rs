/*
last audited 25-07-26 by RSA-Agent (kasirmu-core slice B2: sales deep read; CLI-1 support 25-07-26: new tx-aware create_sale_in_tx + shared validate_sale_money/insert_sale_with_lines helpers); COR-7 re-audited 31-08-26 by docs-auditor (idempotency_key now persisted on both sale paths)
crate: kasirmu-core | status: SAFE | lint: CLEAN
findings: money paths exemplary (MONEY-01..04 + TAX-02..06 in-line, checked arithmetic at every IPC boundary, explicit rounding modes, TAX-06 exclusive-total correction); COR-7 RESOLVED (re-audited 31-08-26, corrected 31-08-26): persisting the idempotency_key column was never the fix — the column existed and was already written on both sale paths (sales_checkout.rs:466/478, sales_lifecycle.rs:463/475); the bug was that nothing populated it and nothing read it back, so a retried checkout inserted a second full payment set. What actually closed it: the desktop command layer derives one key per split as {attempt_id}:{n} from a mount-scoped attempt id (apps/desktop-tauri/src/commands/pos.rs, stamp_attempt_split_keys), and Store::find_sale_by_idempotency_key (db/payments.rs) resolves an attempt key back to its sale so a replay returns the original receipt instead of tripping UNIQUE idx_payments_idempotency_key. The per-split index is load-bearing — a shared key would make one multi-tender sale collide with itself (pinned by complete_sale_deduction_persists_every_split_keyed_to_one_attempt); COR-8 LOW still open: void_sale (sales_lifecycle.rs:567) increments version and guards status='pending', but that is a state guard not the version CAS its comment claims, and the execute() row count is discarded so voiding an already-voided or completed sale returns Ok; COR-9 INFO: receipt-barcode lookup swallows DB errors via .ok(); COR-10 INFO: PartialStockResult travels inside Validation.message JSON (documented tradeoff)
next: check void_sale affected-rows and return Conflict, or add a real WHERE version = ?expected CAS (COR-8) | perf: batch SKU lookup avoids N+1; popularity recompute outside tx
*/
