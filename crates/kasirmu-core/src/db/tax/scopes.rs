//! Scoped tax-rate authoring and effective-rate resolution.
//!
//! The tax-separation P1 cluster: the scope/window model (`TaxRateScope`,
//! `TaxRateWindow`, `TaxRateScopeInfo`, `TaxSaleScope`), the scoped write
//! doors (`create_tax_rate_scoped` / `update_tax_rate_scoped`), the
//! Location → LegalEntity → Global resolver walk, the applicability filter
//! the sale path consults, and the private support types the walk reads
//! through. Plain rate-record CRUD lives in [`super::rates`]; the tier
//! default-clear and last-coverage guard defined here are called from both
//! write paths.
//!
...
impl Store<'_> {
    ...
    pub(super) fn clear_tier_default(
    ...
    pub(super) fn ensure_scoped_coverage_survives(
