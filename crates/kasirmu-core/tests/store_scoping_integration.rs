...
use kasirmu_core::{LocationProfile, Store, migrations};
...
fn apply_wire_snapshot<'a>(conn: &'a Connection, json: &'a str) -> kasirmu_core::Store<'a> {
    let s = store(conn);
    s.create_location_profile(&make_profile("store-a", "Store A"))
        .unwrap();
    s.create_location_profile(&make_profile("store-b", "Store B"))
        .unwrap();
    let snap: kasirmu_core::sync_client::Snapshot = serde_json::from_str(json).unwrap();
    kasirmu_core::sync_client::apply_snapshot(&s, &snap).unwrap();
    s
}
...
fn snapshot_with_unknown_store_id_is_rejected_fail_closed() {
    ...
    let err = kasirmu_core::sync_client::apply_snapshot(&s, &snap);
    let msg = err.unwrap_err().to_string();
    assert!(
        msg.contains("FOREIGN KEY"),
