...
use kasirmu_core::db::stock_transfers::ReceivedLine;
...
use kasirmu_core::StockTransferLine;
...
fn send_transfer_insufficient_stock_fails() {
    ...
    match err {
        kasirmu_core::CoreError::Validation { field, message } => {
            assert_eq!(field, "qty");
