...
findings: API-4 COMPLETE and G-1/G-2/G-3 CLOSED — POST /api/v1/users requires the operator admin key (admin_key_authorised, same second factor as settings/plan/terminal-register/token routes) on top of the JWT, then 403s terminal-scoped tokens as defense in depth (may_manage_users); the C1.1 quota-bypass vector is closed architecturally: staff quota is a licensing-side concern enforced at the license holder (desktop/tablet staff.rs:571/636) and the admin key is the same operator authority as kasirmu-cli, where no quota applies by design; G-2 role_id is validated in Store::create_user/update_user (typed Validation before any write); REV 2026-09-22 (DSH staff-trash): G-3's "staff:delete documented RESERVED" is retired — the key now has three enforcement consumers in kasirmu-bridge (delete/restore/list staff trash), documented at platform/core/src/rbac.rs:408-422; evidence: 11 users tests green incl. create_user_requires_admin_key_when_configured. D1 RESIDUAL CLOSED (2026-09-01): the admin-key write tier now extends to products POST, stock PATCH, tax_rates POST, exchange_rates POST+DELETE via require_admin_write (same operator gate + terminal-scope denial as this route).
*/
//!
