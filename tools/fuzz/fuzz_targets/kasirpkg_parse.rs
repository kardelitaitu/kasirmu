//! Fuzz target for `.kasirpkg` archive parsing (PLG-01/PLG-06 boundary).
//!
//! Feeds arbitrary bytes to `KasirpkArchive::from_bytes` — the same entry
...
//!   `cargo fuzz run --features kasirmu-plugin-fuzz kasirpkg_parse`
...
fuzz_target!(|data: &[u8]| {
    ...
    let _ = kasirmu_plugin::package::KasirpkArchive::from_bytes(data, "fuzz.kasirpkg");
});
