...
It makes its own cargo call with `--all-features` and `--nocapture`, prints `RUN: <passed>; <failed>; <ignored>` read off libtest's own summary, names every skip event it finds, and exits 1. Its PASS requires two independent things — a tree-wide arm census above its floor **and** a proven `--nocapture` capture — so it cannot grade a suppressed log as a clean one. Until this pass, that last clause was the design's whole history: the first draft printed a warning about suppression and **then exited 0**, which is `:64`'s original error rebuilt inside the instrument meant to catch it. Fixed, and now asserted in both directions by `--self-test` (**25 cases, 0 red** at `dd5cf9b9f`; it was 21 when this paragraph was first written, and four more cases went in with the nextest-format diagnosis).
