...
│   ├── mobile-tauri/      # Tablet-optimised Tauri shell
...
| `npm run check:all` | One command — `node ../scripts/check-ui.mjs` (`ui/package.json:11`; no npm-level chaining) — chaining **nine** legs: ESLint → TypeScript → Unit tests → i18n lint → FTL dedupe → Bundle budget → Bundle budget (mobile) → E2E* → Perf smoke, then a `scripts/gates.json` self-audit* (the reading before `b89747b28` said **eight**; that commit added the tablet budget leg between the desktop one and E2E) |
