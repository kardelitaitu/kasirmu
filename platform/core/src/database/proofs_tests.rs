//! Tests for the two skip proofs.
//!
//! A wrong skip applies nothing while reporting success — so every refusal
//! below is asserted together with the control that shows the same statement
//! *is* provable when the proof's conditions hold. A negative test without its
//! control would pass just as happily against a proof that refused everything.
//! The statement layer's own tests live in `statements_tests.rs`.
