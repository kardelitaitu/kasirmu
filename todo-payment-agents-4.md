...
|---|---|
| Phase 0 keys + TerminalFeatureOverride | Rails shipped end-to-end: `get/set_local_payment_methods_scoped` on BOTH clients (desktop `commands/local_payment.rs:41` get / `:62` set, file ends :77; tablet `commands/local_payment.rs:29` get / `:46` set), `ui/src/api/local-payment.ts` (110 lines), settings UI (`ui/src/features/settings/screens/LocalPaymentSettingsCard.tsx`, `.css` alongside). The `crate::feature_key`-namespace variant was SUPERSEDED by this model. |
| Phase 0 EDC device list | `edc_terminals` table + `db/edc_terminals.rs` + boot-time `platform_startup::hardware::register_card_terminals` wired in both `lib.rs` (desktop :200-201, tablet :146) with a `BootstrapReport`. |
...
configured" (`db/edc_terminals.rs:5`, re-read 09-14). Wire it + dropdown +
...
09-14: `drivers/edc/wired.rs` (112 lines) and `wireless.rs` (132) plus
`protocol/{pax,ingenico,verifone}.rs` (46 lines each) all say PLANNED in their
own headers, and `drivers/mock.rs` is the only EDC that can answer — it fails
...
- open_bill as tender — resolved: park action, not a tab (`todo-payment.md:202-208`, the open_bill / credit reconciliation section; re-anchored 09-14 — the old ":152-158" landed inside the mermaid flow diagram, not the decision).
