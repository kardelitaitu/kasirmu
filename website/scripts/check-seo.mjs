...
//  12. headings — exactly one <h1> per page and no level skipped, on EVERY
//      built page rather than only the Astro ones. The outline is how a
//      screen-reader user navigates and how a search engine reads a page's
//      shape, and both read it as a sequence. Four pages failed on the first
//      run — two locale-detect stubs with an empty body, the admin dashboard
...
//      rather than exempted. Only docs-portal/ is exempt: it is generated by
//      mdBook/rustdoc/TypeDoc and staged from public/.
//
...
function parsePage(file) {
  ...
  // Every heading on the page, in document order, for check 12. `<h1>`-`<h6>`
  // only — an `<hgroup>` or an ARIA role is not a heading level, and this is a
...
//     Four pages failed the first run of this check, each for a different
//     reason, and each is fixed rather than exempted:
//       • /index.html and /pair/index.html are locale-detect stubs whose body
//         was EMPTY — a reader with scripting off got a blank page and no
//         links, and the document had no heading at all. Both now carry a
//         noscript fallback with a real h1 and a link per locale.
//       • /admin/index.html had no headings anywhere: every panel is rendered
//         by admin.js into #content, which is also where the skip link points.
//         It now has an sr-only h1 for the region the skip link lands on.
//       • /dev/kds-prototype.html started at h3 and had an empty h2. Its
//         settings sections are h2 now, under a visually-hidden h1.
//     Only docs-portal/ is exempt: it is generated by mdBook/rustdoc/TypeDoc
//     and staged from public/, so its markup is not ours to edit.
for (const page of pages) {
  if (page.url.startsWith('/docs-portal/')) continue;
