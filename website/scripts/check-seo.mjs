...
//  13. accessibility — the non-visual surface: landmarks, an accessible name on
//      every interactive control, ARIA references that resolve, no positive
//      tabindex, no focusable element hidden from assistive technology, and
//      tabs that name a panel. The rule lives in src/lib/accessibility.ts,
//      because src/__tests__/accessibility.test.ts runs the same one on the
//      HTML that ships verbatim and on every template, in `npm test` rather
//      than after a build. Measured first across all 89 pages and 7104
//      controls (2026-09-23): four controls with no name (three settings
//      switches whose visible text sat in a sibling span, one demo input whose
//      only label was its placeholder), three pages with no <main> landmark
//      (a card, a layout div and a full-screen prototype), and two tab lists
//      that announced themselves as tabs with no panel to control. All fixed
//      rather than exempted — and the geometry of the three pages that gained a
//      landmark is pixel-identical afterwards.
//      The exemption is the same page class the heading rule uses: `vendored
//      portal` is generated by mdBook/rustdoc/TypeDoc.
//  14. script controls — the half of 13 that no build output can carry, because
//      a page can be a shell whose controls a script creates. `/admin/index.html`
//      is exactly that (`#content` is filled by admin.js), and the prototypes'
...
const DIST = 'dist';
// This file's own location decides the repo root, so the source walk of check 14
// works from any working directory (check 14 is the one check that reads source
// rather than dist, and it reaches `prototypes/`, which is outside website/).
...
function parsePage(file) {
  ...
  // Every heading the page RENDERS, in document order, for check 12. The rule
  // and the extractor both live in src/lib/heading-outline.ts, because
  // src/__tests__/heading-outline.test.ts runs the same two on the hand-written
  // HTML before this script ever sees a build, and because a heading inside
  // `<noscript>`, `<template>` or a comment is what the extractor has to strip.
  ...
  return {
    file,
    url,
    // The rendered document itself, for the rules that need to walk it (13).
    // Kept raw rather than pre-parsed so the rule stays the only thing that
    // decides what counts as a landmark, a control or a rendered region.
    html,
    main,
...
// 12. Heading structure — exactly one h1 per page, and no level skipped, in
//     RENDERED markup. The rule itself lives in src/lib/heading-outline.ts,
//     because src/__tests__/heading-outline.test.ts runs the same rule on the
//     hand-written HTML (`prototypes/**`, `public/**`) and on every template
//     that writes heading markup, in `npm test` rather than after a build — and
//     because a heading inside `<noscript>`, `<template>` or a comment is
//     precisely what this check exists to reject.
//     Four pages failed its first run, each fixed rather than exempted:
//     /index.html and /pair/index.html (locale-detect stubs whose body was
//     EMPTY, so a reader with scripting off got a blank page), /admin/index.html
//     (no headings anywhere — every panel is rendered by admin.js into #content,
//     which is also where the skip link points) and /dev/kds-prototype.html
//     (started at h3). The exemption is read off the page class rather than a
//     second literal for the same tree: `vendored portal` is generated by
//     mdBook/rustdoc/TypeDoc.
for (const page of pages) {
...
// 14. Script-built controls — the half of check 13 that no build output can
//     carry. `/admin/index.html` ships as an empty shell (`#content` is filled
//     by admin.js) and the prototypes' copy and scroll-to-top buttons are
//     `createElement` calls, so a control with no name there is invisible to
//     every arm above: nothing in dist contains it. The rule is
//     src/lib/script-controls.ts, run here on the SOURCE because that is the
