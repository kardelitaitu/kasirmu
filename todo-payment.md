...
- **Re-deriving this plan's own length, so no future reader inherits a bare number:** `wc -l < todo-payment.md` = **1,389** immediately before this bullet was written and **1,395** immediately after it — an edit to this file moves the number the edit records, measured 2026-09-15 at HEAD `843ceae0d`, which is precisely why the command is the durable part and the figure is not. (No "258" line-count exists in this file to inherit — `grep -n "258" todo-payment.md` returns three hits and all three are substrings of the SHA `ec2edf258` — so a bare 258 seen elsewhere belongs to another document and needs its own command beside it.)
