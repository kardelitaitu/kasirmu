...
pub async fn get_backup_status(state: State<'_, AppState>) -> Result<BackupStatus, AppError> {
    ...
    oz_bridge::data::get_backup_status(&db_path)
        .await
        .map_err(Into::into)
}
...
pub async fn create_backup(state: State<'_, AppState>) -> Result<BackupResult, AppError> {
    ...
    let db_path = state.db_path.clone();
    oz_bridge::data::create_backup(&ctx, &db_path)
        .await
        .map_err(Into::into)
}

/// Adapter over `oz_bridge::data::exportable_settings_rows`; the redaction rule moved
...
fn exportable_settings_rows(rows: Vec<(String, String)>) -> Vec<serde_json::Value> {
    oz_bridge::data::exportable_settings_rows(rows)
}
...
pub async fn export_data(
    ...
    let ctx = state.bridge_ctx();
    oz_bridge::data::export_data(&ctx, &session_token, args)
        .await
        .map_err(Into::into)
}
...
pub async fn import_preview(
    ...
    let ctx = state.bridge_ctx();
    oz_bridge::data::import_preview(&ctx, &session_token, args)
        .await
        .map_err(Into::into)
}

/// AppError adapter over `oz_bridge::data::gate_import_product_batch` — retained
...
pub(crate) fn gate_import_product_batch(
    ...
    oz_bridge::data::gate_import_product_batch(store, products).map_err(Into::into)
}

/// AppError adapter over `oz_bridge::data::gate_import_user_batch`.
#[allow(dead_code)] // retained by the Wave-F extraction contract for sibling tests
pub(crate) fn gate_import_user_batch(
    store: &Store<'_>,
    users: &[serde_json::Value],
) -> Result<i64, AppError> {
    oz_bridge::data::gate_import_user_batch(store, users).map_err(Into::into)
}
...
pub async fn import_data(
    ...
    let ctx = state.bridge_ctx();
    oz_bridge::data::import_data(&ctx, &session_token, args)
        .await
        .map_err(Into::into)
}
...
pub async fn get_backup_status_scoped(
    ...
    let ctx = state.bridge_ctx();
    let db_path = state.db_path.clone();
    oz_bridge::data::get_backup_status_scoped(&ctx, &session_token, &db_path)
        .await
        .map_err(Into::into)
}
...
pub async fn create_backup_scoped(
    ...
    let ctx = state.bridge_ctx();
    let db_path = state.db_path.clone();
    oz_bridge::data::create_backup_scoped(&ctx, &session_token, &db_path)
        .await
        .map_err(Into::into)
}
