...
fn complete_setup_persists_features() {
    ...
    let completed = Settings::get(&conn, kasirmu_core::settings::keys::SETUP_COMPLETE)
        .unwrap()
        .unwrap();
    assert_eq!(completed, "1");

    let preset = Settings::get(&conn, kasirmu_core::settings::keys::STORE_PRESET)
        .unwrap()
        .unwrap();
...
fn complete_setup_persists_all_settings() {
    ...
    let complete = Settings::get(&conn, kasirmu_core::settings::keys::SETUP_COMPLETE)
        .unwrap()
        .unwrap();
...
fn complete_setup_without_transaction_leaves_partial_state() {
    ...
    // transaction, simulating a crash halfway through.
    {
        ...
        Settings::set(&conn, kasirmu_core::settings::keys::STORE_PRESET, "broken").unwrap();
