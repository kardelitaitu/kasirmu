...
> **PROGRESS RECORD (Agent-3, 2026-09-13): T1 contract slice LANDED at `eb4fdd07c1` (health repoint + `From<BridgeError>` seam + oz-bridge dep). The phase remains OPEN — later slices queued (browser+void+scale batch, regional+local_payment, then the bigs: pos/staff/auth/products/settings). Two pre-logged deviations at T1: (1) the allowlist needed NO health-clearing deletions — `version_scoped` is the only health-adjacent tablet entry and it covers a real gap (UI invokes it, tablet registers no scoped health twins), so it stays until a later slice registers them; (2) `Cargo.lock` deliberately NOT committed with T1 — the working-tree lock also carries the cloud-server lane's uncommitted `foundation`/`oz-payment`/`wiremock` rows (agents-1 midtrans work in flight), and committing it would have swept their manifest state under this subject; the tablet `oz-bridge` lock row is content-correct and rides the next lock commit. CONSTRAINT for T2+ (from the T1 baseline-match): tablet `AppState` cannot yet build a full `BridgeCtx` (std `Mutex<Connection>` vs the bridge's `Arc<tokio::Mutex>`, and no cache/plugins/emitter/topology_apply_lock fields), so scoped-command repoints must thread narrower bridge fns or grow tablet-side ctx fields first. Phase-closing note: the sync-conflict parity repair (`ded4686776`) was performed under this phase's fence before T1 — it ports desktop's list/resolve sync-conflict commands into the tablet shell (in-shell, gated `SYNC_MANAGE` on both) and repaired both registration-gate ratchets.
