...
**SIXTH-PASS ACCEPTANCE RE-ATTEMPT — 2026-09-16 (owner "execute the recommended path": run the live-PG acceptance and rename only on real green). Verdict: the green is FLAKY, so `done-` is correctly STILL UNEARNED.** `oz-pg-test-15432` was up (8h), so — unlike the fourth pass — the acceptance is *runnable* on this machine. Ran `cargo test -p oz-cloud-server sync -- --nocapture` **five consecutive times**:

- runs 1, 2, 4, 5 → `test result: ok. 84 passed; 0 failed; 0 ignored`, **zero** skip events;
- **run 3 → one silent skip** (`cannot create throwaway DB`), yet the summary still printed `84 passed; 0 failed; 0 ignored` — because the skip arm returns early, so the test *passes without exercising PG*. That is precisely the hazard `:749` predicted, **reproduced**: the summary line cannot see it, `verify-pg-tests-ran.py` catches it only with a `--nocapture` log it can attribute (my first single-`--log` read it as UNPROVEN; `--assume-nocapture`, which I truthfully met, turned that one green).
