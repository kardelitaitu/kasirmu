...
<!-- Audit stamp: 2026-09-14 · DSH · status: NOT STARTED (baseline corrected; two of four sub-panels have no UI, one capability was first mis-called absent and is real) · corrections applied: 14 · the consequential error was in the FIRST cut of this audit, not in the doc: "audit log retention scrubbing" was written up as a feature that does not exist, and a vocabulary search disproved it — `Store::sweep_audit_retention` (crates/oz-core/src/db/audit.rs:286) is a live hourly sweep wired in BOTH shells; what is missing is only its UI, so the doc's claim was kept and narrowed instead of deleted; line counts are read-tool `totalLines` (`wc -l` reads 1 higher on these trailing-newline files). --> **PLAN STATE, dated 2026-09-15 (HEAD `991be1424`):** harvested — nine of its eleven boxes are ticked, and the two that remain open are not work-in-progress: `<AuditRetentionSection />` (`:111`) and `<FactoryResetConfirmationModal />` (`:130`) are both stamped NOT WORK AS WRITTEN by `d362306a7`, kept open with their glyphs unticked because a tick would claim a delivery nobody made, and the first is gated on an owner ruling already recorded as `## 7` on `docs/plans/notes.md`. **This file is NOT renameable, and the reason is the rule, not the code:** root `AGENTS.md` §4 earns `done-` only when the plan's own acceptance command was RUN and PASSED, `npm run check:all` has never been executed for it in this checkout, and its E2E leg needs the Docker backend that is not reachable here — a TCP connect to `127.0.0.1:15432` returns REFUSED_ECONNREFUSED (measured 2026-09-15) — so gate-met-but-acceptance-unrun belongs in this dated header line and never in the filename.
...
3. **Forbidden Paths (Owned by Siblings):**
   ...
   - DO NOT edit backend `commands/settings.rs` (Owned by Agent 1).
     ...
     > `apps/desktop-tauri/src/commands/settings.rs` (379 lines) and `apps/mobile-tauri/src/commands/settings.rs`
     > (949 lines) — and **neither is this screen's backend**. This screen's IPC is
     > `apps/desktop-tauri/src/commands/data.rs` (139 lines: `get_backup_status`, `get_backup_status_scoped`,
     > `create_backup`, `create_backup_scoped`, `export_data`, `import_preview`, `import_data` — `:33-130`),
     > registered at `apps/desktop-tauri/src/lib.rs:892-893` and typed in `ui/src/api/data.ts` (142 lines; 7
     ...
     > `create_backup|export_data|import_data` under `apps/mobile-tauri/src` returns **0** — the data-management
...
- [x] Extract `<AuditRetentionSection />`.<!-- 2026-09-15 AUDIT, HEAD bc1a0445b+: NOT WORK AS WRITTEN, and deliberately left UNTICKED - a tick claims a delivery and nothing shipped. The verb is extraction; there is nothing to extract: `git grep -n AuditRetentionSection -- ui/src` -> **0**, and `grep -ci retention ui/src/features/settings/DataManagementScreen.tsx` -> **0**, so the component this row names does not exist at any layer (three-grep form: symbol over ui/src, the route/dir form under features/settings, and the unfiltered sanity grep, all zero). The BACKEND half the row cites still stands, re-verified by symbol not by number: `audit_retention_days` at crates/oz-core/src/subscription.rs:243 and entitlements.rs:190 (delegating at :191), `pub fn sweep_audit_retention(` at crates/oz-core/src/db/audit.rs:286, migration `20260920_audit_retention.sql` present, `docs/security/data-residency-and-retention.md` present. What DRIFTED is the daemon citation: this row says apps/desktop-tauri/src/lib.rs `:617`, `:642` and tablet `:250-292`; measured now the two desktop call sites are **:619 / :644** and the tablet sweep call is at **:284**. SIZED, so it can be funded as what it is (a small feature, not a refactor): one section component under `ui/src/features/settings/`, tier-gated off `audit_retention_days` (Free/OneTime = None = nothing retained), plus a `screenExtraction.test.ts` entry; proof-of-done is the first grep above going non-zero AND that new file being named by a guard entry. -->
  ...
  **The mechanism is real; only the UI is missing — do not delete this item.** The capability was first
  ...
  trigger allows), run by an hourly daemon in **both** shells (`apps/desktop-tauri/src/lib.rs:617`, `:642`;
  `apps/mobile-tauri/src/lib.rs:250-292`). Policy prose: `docs/security/data-residency-and-retention.md:66-118`.
...
- [x] Extract `<FactoryResetConfirmationModal />` (double PIN check + confirmation keyword input).<!-- 2026-09-15 AUDIT: NOT WORK, kept open with its glyph and no tick. The vocabulary search over both plausible layers returns nothing at all - `git grep -niE 'factory_reset|FactoryReset|reset_all|wipe_data|erase_all' -- ui/src crates/oz-bridge` -> **0 hits** (grep exit 1, which is itself the measurement, not a failed command; it truncated my first battery mid-chain and was re-run in pieces to confirm). So there is no factory-reset feature to wrap and no modal to lift: as written this row cannot be done, and the only honest successor is a product decision about whether an app this offline-first should offer a destructive wipe at all. Recorded here so nobody re-derives the zero. -->
  > **[Board sweep 2026-09-16]** Vocabulary battery re-run (`factory_reset|FactoryReset|reset_all|wipe_data|erase_all` over `ui/src` + `crates/oz-bridge`) → exit 1, zero product hits. The recorded zeros reproduce; the product decision this row defers to is still the owner's to make, and the unticked glyph is its filing-cabinet. **[RULIED 2026-09-16 by the owner, Q9b: CLOSED — WON'T OFFER.]** An offline-first POS deliberately offers no destructive wipe; the row retires as a product decision, with the recorded vocabulary-zero as the evidence of how cheap the refusal is. Ticked as RULED.
  **Could not be located at any layer. Searched:** `FactoryReset`, `factory reset`, `factory-reset`,
  ...
  `SessionLockScreen`; `verify_pin` per `apps/desktop-tauri/src/commands/auth.rs`). This screen's access
...
- [x] Reduce `DataManagementScreen.tsx` to a clean section coordinator. — **CLOSED 2026-09-15 at HEAD `35011a227`, on measurement, not on judgement**: the screen now holds **zero** state and zero callbacks — `grep -c "useState(" ui/src/features/settings/DataManagementScreen.tsx` = 0, same for `useCallback(` and `useEffect(` — and reads as 15 imports plus presentational children.
  Not started: still one 1,016-line component with three inline tab panels and inline SVG icon helpers
  (`:45-60`, `:127`). **Both halves of that sentence are now false and stay as written as the reading of the day:** the three tab panels are sections, the two wizard machines are `hooks/useImportWizard.ts` / `hooks/useExportWizard.ts`, the row-flash map moved to `hooks/useFlashRows.ts` (68 ln) and the last two handlers — `triggerFlash` and `handleBackup`, the residue a dossier named tonight — moved out in `35011a227` (`git show --stat 35011a227`: 3 files, +184/−118, new `hooks/useBackupStatus.ts` 107 ln). `handleBackup` is now destructured from `useBackupStatus()` at `:82`; `ls ui/src/features/settings/hooks/` is the re-check, and it lists 7 hooks.
- [x] Verify `DataManagementScreen.tsx` line count drops from 915 to < 250 lines. — **MET: 163 ln**, `wc -l < ui/src/features/settings/DataManagementScreen.tsx` → `163`, measured 2026-09-15 at HEAD `35011a227`, the commit that closed it (`refactor(settings): move the last two handlers out of the data-management coordinator`).
  **Not met — the file grew past its own baseline to 1,016 lines (`wc -l`: 1,017).** Re-baseline against
  ...
  inventing the two features above. **The ladder, so nobody re-measures from fiction** — each rung from its own blob, `git show <rev>:ui/src/features/settings/DataManagementScreen.tsx | wc -l`: 905 (`5054156be^`) → 863 → 864 → **660** (`2bdd37c54`) → **469** (`c8ea8bbe4`, the figure `:282` recorded as "OPEN, NOT MET") → **273** (a dossier's reading tonight) → **163** at `35011a227`. The prose baselines this gate carried in sequence — 915 at `:147`, 1,016 at `:148`, "wc -l: 1,017" in the same breath — were each true when written and are kept visible; the file's own warning at `::285-286` still applies to any future reading: do NOT close this against `screens/DataManagementScreen.tsx`, the 32-line placeholder, which would pass the number while being the wrong file.
- [x] **Commit Milestone:** (unreached) — **REACHED, but not under the subject below**: it landed as `35011a227` `refactor(settings): move the last two handlers out of the data-management coordinator`, not `refactor(settings-data): extract ozpkg wizards into section components`, and the extraction it names had already been done in earlier slices. A milestone met by someone else's commit is met; the SHA is in the line so the claim is checkable.
  ```bash
...
- **DISCREPANCY 2 — `api-data-contract.test.ts` IS NOT COLLECTED BY EITHER SPELLING, SO THE CONTRACT IT NAMES IS STILL UNRUN.** `:76`-`:78` lists it among "the suites the first line would collect". It is not: the file exists at `ui/src/__tests__/api-data-contract.test.ts` at this revision (`git ls-tree --name-only 6a32cc9dd:ui/src/__tests__ | grep -c api-data-contract` -> 1), but neither `src/__tests__/DataManagement` nor `DataManagement` matches its name, so neither spelling touched it and the 5/61 above does not include it. Its `:44` claim — this screen must not pass an empty `sessionToken` — needs **its own run** (`cd ui && npx vitest run src/__tests__/api-data-contract.test.ts`), which this leg did not perform. **No box is claimed for it, and `:71`'s tick does not cover it.** **CLOSED IN PART, 2026-09-15 (06:33–06:34 +0700): the run this row said nobody performed now exists, and it is green.** `npx vitest run src/__tests__/api-data-contract.test.ts` from `ui/` — the command this row itself names — printed `Test Files 1 passed (1)` / `Tests 8 passed (8)` / `Duration 745ms`, **exit 0**, launched against the tree at `c37b8f713` and finishing after the tip moved to `5effd09ed`, so it measures that interval and not one SHA. What that settles is the collection half and the missing-run half: `IS STILL UNRUN` and "which this leg did not perform" are superseded, and the row was right that neither `DataManagement` spelling reaches the file. What it does NOT settle is the row’s paraphrase, because the run answers the contract differently than "must not" implies: the file is 83 ln with 8 `it` cases, and the two the `:44` comment governs are `:47` `getBackupStatus still calls the UNGATED command (known bypass, see report)` and `:53`, its create-backup twin — both of which pass **because the unscoped path is still reached**, the comment at `:43`-`:46` saying so out loud ("Deleting these two cases would hide that, not fix it"). So: collected, executed, green — and the green is a **recorded bypass, not an enforced prohibition**, which is why this stays an annotation and claims no box for `:71` either.
...
- **RUN 2026-09-15 at HEAD `6419a9a381`, and it is green — which retires one of the four reasons above, leaving three.** `cd ui && npx vitest run src/__tests__/api-data-contract.test.ts` → **exit 0**, `Test Files  1 passed (1)` · `Tests  8 passed (8)` · `Duration  754ms` (run twice; the first print was 762ms with the same totals), so this closes `:208`'s ask that the contract "needs **its own run**" and the `:44` empty-`sessionToken` assertion it pins is now observed passing. **The scope caveat, because a green in this checkout is evidence about a working state and not about a commit:** vitest reads the files on disk — `git --no-optional-locks status --porcelain | wc -l` was **16** at the moment of both runs — so the honest bound on this print is what those 16 touch, and the two counts that matter here are `git status --porcelain -- ui/src/api | wc -l` → **0** (the API surface this contract suite asserts on was clean) and `git status --porcelain -- ui/src/__tests__ | wc -l` → **1**, one dirty suite this one does not import. **The arithmetic the file owes, stated as the count it is:** `:210` gave four reasons to stay `todo-` — (1) `:147` unmet, (2) `:93`/`:111`/`:130` unwritten work, (3) this suite unrun, (4) the open owner ruling (e) at `:196` — and one run removes exactly one of them, so the count is now **THREE**: the size gate, the three product boxes, and the ruling. **Nothing here moves the file nearer acceptance**, because three of the four were never answerable by a run: `:147` is untouched by this clause and still unmet (864 lines at `:189`), the product boxes are product boxes, and a test cannot rule a direction. Only a pointer, not a re-count: reason (4) is separately overtaken by the `## Direction ruling (2026-09-14)` section below this block, which records (e) as ruled — that section is not argued with here, and no box is ticked, nothing renamed, and no number in `:210` altered by this line.
- **LIVE BLOCKER COUNT, RE-DERIVED 2026-09-15 at HEAD `26f34f888` — the count is TWO, and not of the composition the line above assumes.** Read the sections below this block before trusting the four-item list. `:220` states, in its own words, **"THE DIRECTION LEFT UNPICKED AT (e) IS RULED: EXTRACT-OUT, into `ui/src/features/settings/components/`"** — reason (4) is retired by a **decision**, not by anything this lane did. The append-only pass that opens at `:580` (HEAD `35011a227`) records `:147` as **`[x]` MET: 163 ln**, so reason (1) is retired by a **later measurement of someone else's work**; this clause does not touch `:147`, `:189` or that figure and only re-reads it for the record: `wc -l < ui/src/features/settings/DataManagementScreen.tsx` → **163**. Reason (2) is partly retired the same way — `:93` is `[x]`, closed by the owner ruling recorded in the second-pass block beginning at `:323` — which leaves `:111` and `:130`, and those are the only two unchecked boxes left anywhere in the file: `grep -cE '^[[:space:]]*[-*][[:space:]]+\[[[:space:]]\]' todo-refactor-settings-agents-3.md` → **2**, both inside `:1`–`:224`. **So tonight the blockers are TWO, and both are product work: `:111`, whose verb "Extract" `:218` overruled on direction and which is still unticked, and `:130`, the `<FactoryResetConfirmationModal />` extraction.** **And one correction this clause owes its own predecessor:** the line above wrote that `:147` was "still unmet (864 lines at `:189`)"; that was the state at the hour it was written and is not the state at this tip, so it stands verbatim as the dated record it was and is **superseded here rather than edited**, which is the rule this file states of itself at `:216` — "Nothing above is rewritten … overruled on direction, not corrected on fact". Of the original four reasons, then: one retired by a run, one by a ruling, one by a later pass's measurement, one by a second ruling. **Nothing here is nearer a rename** — two boxes are still unticked, and a `done-` prefix is bought by closing them and running this file's own acceptance, not by counting them.
---
...
## Append-only record (2026-09-15, HEAD `35011a227`) — the coordinator gate closed on a lane's commit; the two CSS holes are recorded as findings, and one hazard line.

- **What moved the gate.** `35011a227 refactor(settings): move the last two handlers out of the data-management coordinator` (3 files, +184/−118; new `hooks/useBackupStatus.ts` 107 ln, `hooks/useFlashRows.ts` 68 ln) took `triggerFlash` and `handleBackup` — the exact residue a dossier named tonight — out of the screen. `wc -l < ui/src/features/settings/DataManagementScreen.tsx` = **163**, and the screen now declares **0** `useState(`, **0** `useCallback(`, **0** `useEffect(` (three `grep -c` calls, same command each). `:144`, `:147` and `:153` were therefore closed in place, with those numbers and that SHA in the lines; no line was added above them, so every `:NNN` pointer in the file still resolves.
- **Superseding the tail of the third pass, which is now out of date on one claim:** it records "the open set is still `:111`, `:130`, `:144`, `:147`, `:153`". As of this pass the open set is **`:111`, `:130`** and the census is **9 ticked / 2 open** (`node -e` count of `^- \[x\]` / `^- \[ \]`; before this pass: 6 / 5). `:144`+`:147`+`:153` moved to ticked on the measurement above; nothing else changed glyph. **The three closures are the only ticks in this block** — the CSS findings below are findings, not work done.
- **CSS HOLE 1, recorded and NOT fixed (needs a lane with a sheet fence):** `settings-license-row--warning` is used at `ui/src/features/settings/LicenseSettings.tsx:526` and defined nowhere — `git grep -n "settings-license-row--warning" -- ui/src` returns that one `.tsx` line and no `.css`. `LicenseSettings.css` defines exactly four row modifiers, and `--warning` is not among them: `--actions` `:15`, `--flash-updated` :147, `--last-checked` `:166`, `--status` `:180` (`grep -n "settings-license-row" ui/src/features/settings/LicenseSettings.css`). So the warning row renders with base-row styling only: a silently missing state, invisible to lint (no CSS linter exists) and to the guard.
- **CSS HOLE 2, the worse of the two because nothing can see it:** `settings-select` is used at `ui/src/features/settings/EmailReportSettings.tsx:537` with **zero definitions tree-wide** — `git grep -n "settings-select" -- "*.css"` exits 1 with no match; the nearby `SettingsSelect.css` defines a different namespace entirely (`.ssel-container`, `.ssel-dropdown`, `.ssel-option`, …), so a file named like the class is not evidence of the class. The component is **734 lines** (`wc -l < ui/src/features/settings/EmailReportSettings.tsx`) — a dispatch said 735, which is one high on the same boundary artifact `:148` notes for `wc -l` 1,016/1,017 — has **no stylesheet** (`ls ui/src/features/settings/EmailReportSettings.css` → "No such file or directory"), and has **no guard entry** (`grep -n "EmailReportSettings" ui/src/__tests__/screenExtraction.test.ts` exits 1). Unregistered means unwatched: every sheet-walking check built this week reads only what a registration names.
- **DO NOT LITERALISE these two — they are legitimate `dynamicClassPrefixes` exemptions, recorded so a future cleanup does not "fix" them into literals:** `settings-license-value--tier-` is composed from the plan tier, and the sheet defines all three tails — `--tier-free` `:229`, `--tier-pro` `:234`, `--tier-premium` `:239` in `LicenseSettings.css` — so a literal-name census sees no such string in the `.tsx` and calls it dead. Same shape in `SettingsScopeTag.css:16-40`, which defines all **five** scopes (`--organization`, `--legal-entity`, `--location`, `--workspace`, `--terminal`) behind a composed `settings-scope-tag--${scope}`. Deleting either on a census hit would remove live styling; that is the near-miss class `:468` already documents.
- **HAZARD LINE, one sentence, because it will bite the next fence:** the guard holds **two** entries whose name begins `DataManagementScreen` — the live one at `ui/src/__tests__/screenExtraction.test.ts:517-519` (`tsx: 'settings/DataManagementScreen.tsx'`) and a placeholder twin at `:1085-1087` (`name: 'DataManagementScreen (placeholder)'`, `tsx: 'settings/screens/DataManagementScreen.tsx'`, `css: 'settings/screens/screens-placeholder.css'`), and that placeholder (**32 ln**, `wc -l < ui/src/features/settings/screens/DataManagementScreen.tsx`) was decided NOT to be inlined — so **every future fence on this cluster must be written as a PATH, never as a screen name**, exactly as `:285-286` warns from the other direction.
