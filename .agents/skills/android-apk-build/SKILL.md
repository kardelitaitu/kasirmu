...
**Cause 3 — the dev server bound to loopback, so the tablet cannot reach it (measured 2026-09-19; the
config was FIXED on 20-09-26 by `f1a1a2cec`, after this section was written).**
A `dev` build loads `build.devUrl`, whose host the CLI rewrites to this machine's LAN address
(`http://192.168.0.168:1422`). `ui/vite.mobile.config.ts:142` is now `host: host || '0.0.0.0'`, where
`host = process.env.TAURI_DEV_HOST` — the fallback binds **all** interfaces, so a hand-started server is
reachable from the tablet with no env var set. It was `host: host || false` until `f1a1a2cec`, and
`false` means `localhost`, which on Windows is `::1` alone — nothing was reachable. Read line 142 before
assuming either behaviour: on a branch that predates that commit, or on a host whose config overrides the
fallback, the original failure is still live, and it presents identically. `cargo tauri android dev` sets
`TAURI_DEV_HOST` itself either way, so the CLI's own frontend server was always fine; one you start by
hand with `npm run dev:mobile` was not, and now is.
...
`kasirmu_bridge::auth::check_username` (`crates/kasirmu-bridge/src/auth.rs:975`; its doc block is
:963-972 — re-measured 22-09-26, the stamp that first recorded this cited `:856`) is deliberately a
...
### `cargo tauri android build` DOES build the frontend — the "0 hits under `mobile/`" test was a false negative
...
1. **Source, tauri-cli 2.11.4.** `run_hook` *is* absent from `mobile/`, because the mobile path does
   not call the hook itself: `mobile/android/build.rs:197` calls
   `crate::build::setup(&interface, &mut build_options, tauri_config, dirs, true)`, and `setup` — the only
   `pub fn` in that region of `build.rs` — is where `run_hook("beforeBuildCommand", …)` sits
   (`build.rs:187-196`). A grep scoped to the directory misses a call made one level up.
