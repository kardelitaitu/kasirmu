...
- **`.githooks/pre-commit`** runs on every commit. **Source of truth is the hook itself — `grep -n '^# ──' .githooks/pre-commit` — and [`AGENTS.md`](../../../AGENTS.md) § Quick Setup enumerates every step with its rationale.** As of 0.0.37 there are ten: cargo fmt, LF normalization, i18n lint, staged bundle parity, FTL dedupe, migration column-type lint, PG schema drift guard, Go, UI typecheck, and FTL orphan lint. This line previously listed eight and went stale twice; `scripts/verify-agents-mirrors.py` now polices skill files against the hook, so a restated count that disagrees is a red build rather than a wrong belief.
