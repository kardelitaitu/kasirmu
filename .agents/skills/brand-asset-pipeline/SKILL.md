...
<!-- Audit stamp: 2026-09-22 · Budak-Korporat · status: ACCURATE — 0 findings · Audited against branch `0.0.39` at `e56bf8307`, working tree clean. Every load-bearing claim re-measured, none inferred from the previous stamp. Verified this pass: `scripts/sync-branding.ps1` present (19,312 B); `assets/branding/` holds four tenants (`acme-tenant`, `beta-retail`, `default`, `whitelabel`) and `default/` has all four subdirs; `manifest.json` declares `appName`/`companyName` `kasir.mu`; the identifiers really are `mu.kasir.app` (desktop `tauri.conf.json:5`) and `mu.kasir.mobile` (mobile `:5`); `ui/src/features/design/brand-tokens.css` exists; `assets/source-icon.png` exists; `assets/branding/default/vector/` carries exactly the four variants and `ui/public/branding/` mirrors all four; `StaffLoginScreen.tsx:368` really does load `/branding/logo-mark.svg`; `ui/vite.config.ts` sets **no** `publicDir` (grep returns nothing), so the `public/`-is-verbatim trap stands; `docs/decisions/2026-07-15-whitelabel-branding-system.md` exists; `website/public/` has `favicon.svg`, `og-image.svg`, `og-image.png`. Counts re-counted from the tree: the Windows-Store set is exactly the 9 `Square*.png` + `StoreLogo.png` = 10, and `icons/ios/` holds exactly 18 files. `ui/public/site.webmanifest` declares all four PWA icons in the documented order. Hardware PNGs decoded from their IHDR: `receipt-logo-58mm.png` 384x100 depth 1, `receipt-logo-80mm.png` 576x150 depth 1, `invoice-watermark.png` 512x512 grayscale-alpha — all three match the stated spec. The trap-5 repair was confirmed to still hold: all six `assets/branding/default/desktop/*.png` (incl. `tray-icon.png`) decode as **depth 8, colour type 6**, and `apps/desktop-tauri/icons/128x128@2x.png` is 256x256 depth 8 — byte-consistent with the "comes from `desktop/256x256.png`" claim — while `icons/icon.ico` is 18,291 B, matching trap 3's "~18 KB". Not re-measured (needs ImageMagick and/or a device): the Q16 rasterisation behaviours, the `.icns` packer, and the tauri runtime panic text. -->

<!-- Superseded audit stamp: 2026-09-19 · Budak-Korporat · status: ACCURATE at audit time (0 findings on 22-09-26; kept verbatim — this file preserves superseded readings) · Derived from a full regeneration of the `default` brand on 2026-09-19 (commits ac25c3e32, df5e0ff87, ea3aa2c7e, ea60a3c16) plus the icon.ico decode fix (32ea54472) and the hardware rebrand (6c09d4e1e). Verified this pass: `scripts/sync-branding.ps1` parses clean under PowerShell (PARSE_OK via the parser) and its dry-run runs; `assets/branding/default/manifest.json` declares appName kasir.mu; both app `tauri.conf.json` files end a sync run with a zero diff; `ui/public` and `assets/branding/default/web/` agree byte-for-byte on all six web icons; the logo exports in `ui/public/` are the designer's own files and `ui/public/branding/` holds the four vector variants. The 16-bit ICO trap was confirmed live (cargo check panicked, then passed on the 8-bit file). The hardware receipt logos were regenerated with `-resize WxH! -threshold 50%` (the `!` was a doc bug in the README/script, now fixed) and the watermark reproduced at ~19% mean alpha (measured 13728 vs the old 12600). The ImageMagick and PowerShell behaviours below were measured on this host, not inferred. · Pass 2 (2026-09-19, Android build-repair lane): trap 5 added — the 16-bit **PNG** icon panic. Measured end to end on the Redmi 23073RPBFG: with `windows: []` the bug was silent, and once a window was declared every launch died at `tauri-2.11.3`'s `app.rs:1425` with `invalid icon: The specified dimensions (32x32) don't match the number of pixels supplied by the `rgba` argument (2048)`, surfaced as `SIGABRT` and a MIUI `thirdappassistant` crash dialog. The six `default/desktop/*.png` were re-encoded to 8-bit `TrueColorAlpha`, `sync-branding.ps1 -Brand default` propagated them, all 18 destination files verified byte-identical to source, and the app then installed, launched, stayed resumed and rendered (815 distinct colours, no panic). Also confirmed this pass: on a clean tree the sync leaves both `tauri.conf.json` files and `ui/public/site.webmanifest` byte-identical and touches only the timestamp line of `brand-tokens.css`. Left open and NOT this pipeline's bug: with `useHttpsScheme: true` the dev page loads over `https://tauri.localhost`, so Vite's HMR client throws `SecurityError: Failed to construct 'WebSocket'` — a dev-only hot-reload failure owned by `ui/vite.mobile.config.ts`'s `hmr.protocol`. -->
