...
<!-- Audit stamp: 2026-09-22 · Budak-Korporat · status: REPAIRED — 1 finding, fixed in place · Audited against branch `0.0.39` at `e56bf8307`, working tree clean. · F1 (MEDIUM, fixed): the test-suite counts went stale. The line said "297 files, plus 255 `.test.ts`"; re-counted this pass, `ui/src/__tests__` holds **321** `.test.tsx` and **598** `.test.{ts,tsx}` — i.e. 277 non-`.tsx`. Corrected, and the line now says to re-count rather than quote, because the suite grows weekly and a stale count is exactly the drift this is. · F2 (confirmed REPAIRED, no action): every path drift the 18-09-26 audit raised against this file is gone. The dead pre-P9a locale directory under `ui/src/` is no longer cited anywhere in the body — the corpus is `shared-ui/locales/`, which exists; (this stamp names it descriptively rather than literally because Check 1 does not strip HTML comments, so quoting a dead path here would flag this file); the `frontend/themes/tokens.css` contradiction is resolved — both the remaining reference and the exit-animation skill point at `ui/src/theme/tokens.css`, which exists; `@/locales/test-utils` is gone in favour of `ui/src/__tests__/test-utils/render.tsx`, which exists. · Verified this pass: all 14 paths this file cites exist — `shared-ui/locales/`, `ui/package.json`, `ui/src/{__tests__,api,contexts,i18n,theme,types,utils}`, `ui/src/__tests__/test-utils/render.tsx`, `ui/src/api/tauri.ts`, `ui/src/i18n/{index.ts,LocaleContext.tsx}`, `ui/src/test-utils/renderInAct.ts`, `ui/src/theme/tokens.css`, `ui/src/types/domain.ts`, `ui/src/utils/logged-invoke.ts`, `ui/tsconfig.json`. · NOT re-measured: the rendered-output and accessibility claims, which need a Vitest run this audit did not perform. -->

<!-- Superseded audit stamp: 2026-09-03 · DSH · status: ACCURATE at audit time (superseded by the 2026-09-22 stamp above) (rev 2) · fixes over rev 1: F1 locale paths (per-feature bundles, en|id), F2 token path (ui/src/theme/tokens.css), F3 state libs (no TanStack/Zustand — removed), F4 ci.yml act-gate reference removed (file does not exist; renderInAct guidance kept on its own merits) · added: design-language reference section, real token families, motion & feedback rules, real test render helpers, data-testid convention · verified this pass: per-feature .ftl/.id.ftl bundles + shared.ftl/bundles.ftl + locales/index.ts, LocaleCode 'en'|'id' in ui/src/i18n/index.ts, dark-default tokens.css (:root dark / [data-theme="light"]), --color-*/--space-*/--radius-*/--shadow-*/--duration-*/--ease-*/--z-*/--font-* token families, api/tauri.ts sole @tauri-apps re-export + utils/logged-invoke.ts, ~40 per-domain ui/src/api/ modules (pos.ts is one of many), formatMoney in types/domain.ts (id-ID default), flat ui/src/__tests__/ with renderWithFluentSync/renderWithFluent/renderWithProviders(Sync)/rerenderWithProviders + renderInAct + withFluent, React 18.3.1, @fluent/react, strict tsconfig, no external state library in package.json, all FTL ids used in examples exist in sales.ftl -->
...
Component tests live in `ui/src/__tests__/` as **flat** `<Component>.test.tsx` files (**321** files, plus **277** `.test.ts`, e.g. `CartScreen.test.tsx`, `PaymentModal.test.tsx`). Both figures re-counted on 22-09-26 with `find ui/src/__tests__ -name '*.test.tsx' | wc -l` (321) and `find ui/src/__tests__ \( -name '*.test.ts' -o -name '*.test.tsx' \) | wc -l` (598, so 277 non-`.tsx`); this line used to say 297 and 255. Re-count rather than quoting these — the suite grows weekly. Shared render helpers are in `ui/src/__tests__/test-utils/render.tsx`:
