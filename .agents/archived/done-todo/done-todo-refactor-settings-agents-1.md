...
**Goal:** Decompose `apps/desktop-tauri/src/commands/settings.rs` (1,109 lines) into modular sub-modules: printer settings, receipt layout configurations, currency/tax defaults, and local API keys.

**Target File:** `apps/desktop-tauri/src/commands/settings.rs`  
**Sibling Documents:**
...
2. **Owned Path Fence (Exclusive to Agent 1):**
   - `apps/desktop-tauri/src/commands/settings.rs` & `settings_tests.rs`
   - Submodules under `apps/desktop-tauri/src/commands/settings/` (NEW)
