...
|---|---|
| Phase 0 keys + TerminalFeatureOverride | Rails shipped end-to-end: `get/set_local_payment_methods_scoped` on BOTH clients (desktop `apps/desktop-tauri/src/commands/local_payment.rs:41` get / `:62` set, file ends :77; tablet `apps/mobile-tauri/src/commands/local_payment.rs:29` get / `:46` set), `ui/src/api/local-payment.ts` (110 lines), settings UI (`ui/src/features/settings/screens/LocalPaymentSettingsCard.tsx`, `.css` alongside). The `crate::feature_key`-namespace variant was SUPERSEDED by this model. |
| Phase 0 EDC device list | `edc_terminals` table + `crates/oz-core/src/db/edc_terminals.rs` + boot-time `platform_startup::hardware::register_card_terminals` wired in both `lib.rs` (desktop `apps/desktop-tauri/src/lib.rs:203` - moved off :200-201 by 09-15 - , tablet `apps/mobile-tauri/src/lib.rs:146`) with a `BootstrapReport`. |
