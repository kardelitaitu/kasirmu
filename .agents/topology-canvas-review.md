...
> **RESOLVED 2026-09-16 — owner order "lets fix it"; executed in `ec7b63175`.** Envelope + counter are now per-branch slices mirroring `topology_setting_key`, a never-saved named branch answers `null` like the real command, the dependent revision-browser scaffold was fixed in the same commit, and three isolation cases in `dev-mock-stores.test.ts` pin the properties. The section below stands as the finding it was; the divergence is closed.
