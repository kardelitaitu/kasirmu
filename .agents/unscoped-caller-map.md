...
| 8 | set_setting (D+T) | api/settings.ts:230 | :247 yes | UpdateBanner.tsx:211 (unconditional, by design) · 5 scoped sites | both | 5 files | UNSC |
...
| 25 | export_daily_summary (T) | sales.ts:704 | :708 yes | none — DailyTotalWidget.tsx:65 uses the **twin** | unscoped only in a test mock (mocks/api.ts:64) | 0 | **NOCALL** |
| 26 | export_sales_by_hour (T) | sales.ts:712 | :716 yes | none — SalesByHourWidget.tsx:60 uses the twin | unscoped only in mocks/api.ts:65 | 0 | **NOCALL** |
...
| 33 | get_default_currency (T) | api/currency.ts:34 | :42 yes | contexts/CurrencyContext.tsx:51 (unconditional) and :69-71 (ternary) · PaymentModal.tsx:278 uses twin | both | 3 | UNSC |
...
`ui/src/api/settings.ts:230` `loggedInvoke<void>('set_setting', { key, value, userId })` vs twin `:247` `'set_setting_scoped'`. Twin callers: ExitSurveyModal.tsx:52, EmailReportSettings.tsx:163, hooks/useSettingsSave.ts:199-200 (name anchor `setSettingScoped(sessionToken, 'sync.auth_token'`; the call sits inside the `if (saveResult('sync'))` branch opened at `:195` of that 253-line hook), GeneralSection.tsx:50, WorkspaceKds/Inventory/RestaurantPos batch saves. **That third entry is a FILE RELOCATION, not line drift** — the save orchestration moved verbatim out of `SettingsPage.tsx` (its `:30` now imports `./hooks/useSettingsSave`; comment at its `:399`), so on the 820-line page `:479` is a lone `}` and a reader searching for the old number alone finds nothing. The one unscoped caller is UpdateBanner.tsx:211 inside `persistUpdaterSetting`, whose comment at `:205-208` says the unscoped command is used **deliberately** because the banner renders before login. So this name cannot be deregistered by editing a ternary — it needs either an unauthenticated-by-design allowance or a dedicated `updater.*` command. It writes `updater.previous_version` / `updater.last_backup_path` (UpdateBanner.tsx:13,16).
