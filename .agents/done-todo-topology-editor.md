...
**Fence:** `crates/oz-bridge/src/topology/commands.rs`, `crates/oz-bridge/src/topology/persistence.rs`, `apps/desktop-tauri/src/commands/topology/topology_command_tests.rs`
...
- [x] Apply the swap at `commands.rs:555` to open `effective_store_id`. **HOLD -- measured at `bde88f7e2`, this swap regresses a documented design.** The site is not an ownership comparison: it opens the session store DB to supply the SECOND REGISTRY to `validate_apply_gate(&[&global_db, &branch_db], ...)` at `:561`, and the comment block at `:547-552` says the either-registry read exists because scoped creates land branch profiles in the SESSION store database, so validating the global registry alone "rejected every freshly created branch with `unknown-branch-location` forever". Pointing it at `effective_store_id` alone re-creates exactly that bug. The same is true of the second site the first pass found, `:938`, which opens the session store DB to pass `Some(&branch_db)` into `save_topology_json_at_key_with_revision` (`:944-954`) -- a save-side registry, not a gate. Every real ownership comparison in the body already uses `effective_store_id` (`:614`, `:701`, `:746`, `:774`), as do the mutation (`:677`), the recovery journal (`:526`) and the audit write (`:1021`), so the divergence F1 names is between WHICH DATABASE IS CONSULTED, not between two authorities about who may write.
      - **EXECUTED 2026-09-16 at `32abb9cbb` — the HOLD's own analysis turned out to be the fix's shape.** R4 ratified (ruling block, §5), then the FACT clause decided the literal form: the swap as written (point the `:555` open at effective ALONE) was NOT applied — the goal authorized choosing between the registry slices by established fact, and the fact is that scoped creates write the profile row into the SESSION registry (pinned green by the bridge's own `create_location_profile_scoped_end_to_end_owner`, `locations_tests.rs:148-154`: count == 2 after create) while a new store db seeds only 'default' — so `[global, effective]` alone WOULD have recreated the forever-reject this box warned about. The alignment ADDS the effective store's registry to both sites that model ownership by registry membership: the gate (`validate_apply_gate([global, session, effective])`) and — found red-first, not theorized — the SAVE boundary, whose `Some(&branch_db)` `[global, session]` re-check killed the referee test even AFTER the gate was aligned: the second site was live, and only a passing-through test found where it actually died. Consequences: a self-describing store's own registry can now authorize an Apply into it (T1 `self_describing_store_passes_the_ownership_gate`, the plan's false-reject direction at :193, red with `unknown store_profile_id: char-gate-self` → green), fresh-create stays alive (T2, green before AND after by design — the guard's job is to make a wrong fix fail), and the ANY-registry residual (a session-side row still authorizing writes into a target that does not name itself) is pinned DOCUMENTED-ACCEPTED (T3), because full closure needs a write-side self-seed in the §I locations family (outside this fence) or dropping the session arm (ruled out by the fact above). R4(a) needed no code: the EXISTING char harness already proves the scoped user naming store_b is refused 'out of scope' (grants on the named store demanded today). The gate block's comment is rewritten to state the three-registry design with the residual named inline; the new-store-file side effect (`open_store` creates-and-migrates, so a rejected unknown-id diagram can now leave an empty store-<id>.sqlite — file-only, zero rows, the same shape `create_store_db` produces) is recorded at the site. Acceptance prints: referee red first (`61 passed; 1 failed`), then after the save-site fix `cargo test -p oz-pos-app --lib topology` `62 passed; 0 failed; 0 ignored` (57.94s), `cargo test -p oz-bridge topology` `314 passed; 0 failed; 0 ignored` (6.94s — the 40-odd test callers of the old single-registry save signature ride a thin delegating adapter, zero test edits), `verify-ipc-parity.py` exit 0 (nothing on the command surface moved; the box's own word for it: tripwire), `rustfmt --edition 2024` clean scoped to the three files (workspace-wide fmt remains forbidden while other lanes are mid-edit — and the edition matters: the let-chains in these files reject 2021).
      - **DISPOSITION 2026-09-15 ~13:15 at tip `960d00568` · CLASS: NEEDS RULING.** Cites `docs/plans/notes.md` item 20 ("Does a global grant authorise writes into whichever store a request names?"), which is this file's section 5 item 4 and is UNANSWERED. The site is confirmed live, so the ruling is the only blocker: `grep -rn 'validate_apply_gate' crates/oz-bridge/src/topology/commands.rs apps/desktop-tauri/src/commands/topology/commands.rs` -> one hit, `crates/oz-bridge/src/topology/commands.rs:561`, and `sed -n '555p'` there prints `let branch_conn = ctx.db_manager.open_store(&session.store_id)` -- i.e. the row's `:555` still points where it says. `crates/oz-bridge/src/topology/commands.rs` and the app-layer `commands.rs` are both **clean** at this tip (`git status --porcelain -- apps/desktop-tauri/src/commands/topology/` -> no path), so the earlier briefing that the app file was mid-edit has aged out; the block is the answer, not the file.
- [x] Re-read and correct the comment at `:547-552` to match the code's actual registry choice.
...
- [x] Run `python scripts/verify-ipc-parity.py` → `IPC parity: OK` (no IPC surface changed, so this is a tripwire, not a fix).
...
- [x] Commit with the pathspec form and report the hash.
...
**Fence:** `crates/oz-bridge/src/topology/commands.rs`, `apps/desktop-tauri/src/commands/topology/commands.rs`, `ui/src/api/topology.ts`, plus the IPC parity allowlist if a signature change requires an entry.
...
| Desktop shim | `apps/desktop-tauri/src/commands/topology/commands.rs:132-139` — signature is `(branch_id, state)`, no `session_token` |
| UI wrapper | `ui/src/api/topology.ts:60-64` — matches |

Not registered on the tablet (`grep -rn "load_topology\b" apps/mobile-tauri/src/` → 0).
...
- [x] **Blocked:** obtain the ruling in §5 item 1. Do not guess at this one.
...
- [x] If the ruling is *add a session*: thread `session_token` through all three layers. The shim change is mechanical; the parity gate confirms the registration.
...
- [x] If the ruling is *intended*: write the reason next to the command, in the same voice as the template check's comment, and say explicitly why the diagram needs less protection than the template — because the file currently argues the other way and a future reader will otherwise "fix" it.
...
- [x] Either way, add the test that pins the chosen behaviour, so the decision is not re-litigated by the next reader.
...
- [x] Run the Phase 2 acceptance pair and commit.
      ...
      - **PHASE 2 EXECUTED 2026-09-16 at `8286f43ae` (all five boxes, ruling R1 of the §5 block; the "intended" branch above was cancelled by that ruling — the comment fix is dead, the three-layer change is the law).** `load_topology` takes `session_token: String` first in the bridge (resolves the session BEFORE touching settings — an unauthorized caller learns nothing about whether a diagram exists, in any branch), the desktop shim forwards, and `ui/src/api/topology.ts` threads it: the wrapper's old quirk (whole payload `undefined` when no branch) is gone — the wire now ALWAYS carries a payload object, a fact both api contract tests now pin with named session/branch cases. Reach: one lifecycle hook (new `sessionToken` dep, added to its effect deps) fed from the editor's existing `useWorkspace` token, three TopologyScreen sites (two gained the house `if (!sessionToken) return;` guard, the history button degrades to opening without the snapshot — the same path a failed fetch already takes). Nine desktop command tests gained a documented `seed_topology_session` helper (constant token: the gate checks existence+liveness, not identity). Two-stage as ever: stage 1 wired the parameter and left the body gate-free — `58 passed; 1 failed`, the failure exactly at the R1 refusal assertion (behavior red, not compile red; note the compile detour cost two runs: `&State<AppState>` does NOT coerce to `&AppState` through a method receiver's generic, `app.state::<AppState>().inner()` is the door); stage 2 enforced — `59 passed; 0 failed; 0 ignored` (57.19s), `cargo test -p oz-bridge topology` `314 passed; 0 failed; 0 ignored`, `verify-ipc-parity.py` exit 0 OK (names-only — no allowlist entry needed), `npm run typecheck` clean, 6 UI suites 145/145. The dev-mock handler STAYS permissive on purpose (T-4 convention: it models state, not auth) — the two dev-mock-driving tests pass a literal token and note that in-place.

---

## Phase 3 — F2 + M3: one scope-policy decision, applied twice

**Fence:** `crates/oz-bridge/src/topology/commands.rs`, `apps/desktop-tauri/src/commands/topology/topology_command_tests.rs`
...
- [x] Decide the policy once: is topology a global admin tool (probe is right, write path is over-scoped) or a location-scoped one (probe is wrong)? **Write the answer down before changing code**, because this is the same question asked twice and a per-site answer guarantees the contradiction returns.
...
- [x] Apply the decision to `can_save_topology` (`:33-49`) — either make the probe take the branch it is probing for and use the scoped check, or state in both places why it is intentionally broader.
...
- [x] Apply the same decision to `pin_topology_revision` (`:252-256`).
...
- [x] Run the acceptance command and commit.
      ...
      - **PHASE 3 EXECUTED 2026-09-16 at `4efcb0971` (all five boxes above, ruling R2 of the section-5 block).** Two-stage test-first landing: stage 1 wired the `branch_id` parameter through bridge `can_save_topology`, the desktop shim, and `ui/src/api/topology.ts` while BOTH bodies kept the scope-free check — the new agreement test then failed exactly where R2 says the disagreement lives (`57 passed; 1 failed`, panic at the probe-must-deny assertion; the four older role-based probe assertions stayed green, so the fixture proves the delta is scope, not role). Stage 2 swapped both bodies to `require_user_permission_scoped(..., branch_id, None)` and rewrote both stale comments to cite the ruling — `58 passed; 0 failed; 0 ignored` (57.38s). Pin's fix was body-only: the parameter already ran end-to-end. Acceptance prints at the wired state: `cargo test -p oz-bridge topology` -> `314 passed; 0 failed; 0 ignored` (baseline held); `python scripts/verify-ipc-parity.py` -> `IPC parity: OK` exit 0 (the gate compares command NAMES, so the arg change needed no allowlist entry); `api-topology-contract` + `api-ipc-contract` + `dev-mock-stores` -> 63/63 (the wrapper's added `branchId: undefined` key passes the frozen payload pins by undefined-equality — a fact about vitest the next wrapper change should re-verify, not assume). Honest reach note: `TopologyScreen.tsx:130-136` has gated CLIENT-SIDE since well before this phase (it never consulted the probe on modern trees), so F2's user-visible scenario — author a diagram, die at Apply — was already unreachable from the screen; what this phase fixed is the registered command's ANSWER, which any future or foreign caller (the tablet shell, an API consumer) would otherwise have received wrong. The disagreement was real; its blast radius was smaller than F2's prose implied, stated here so the fix is never quoted as bigger than it is.

---

## Phase 4 — F4: pin the idempotency ledger

**Fence:** `apps/desktop-tauri/src/commands/topology/topology_command_tests.rs`
...
- [x] Test 1: apply once with a given `request_id`, apply again with the same id and an identical payload. Assert the second returns the **original** revision rather than creating a new one, and that no second revision row exists.
      - **DISPOSITION 2026-09-15 ~13:15 at tip `960d00568` · CLASS: ALREADY PAID.** `b2797ba7e test(desktop-tauri): cover topology replay by request_id` (+**223**, 2026-09-15) landed `a_retried_request_id_answers_from_the_ledger_without_repeating_the_mutation` at `apps/desktop-tauri/src/commands/topology/topology_command_tests.rs:2071`, on the shared `replay_apply` harness at `:2028`. Not ticked by this lane for the same reason as every other run-box: no cargo was run here, so the green is attributed to the commit and to item 20's record, not re-derived.
      - **PAID 2026-09-15 ~16:35 at tip `75e5e5432` · TICKED — CLASS: ALREADY PAID by the named commit.** `b2797ba7e test(desktop-tauri): cover topology replay by request_id` (+223/-0, same file) is Test 1, replay of an identical payload returning the original result; re-confirmed present at this tip.
...
**Fence:** `apps/desktop-tauri/src/commands/topology/topology_command_tests.rs`, `ui/src/__tests__/topologyExport.test.ts`, `ui/src/__tests__/topologyKindRegistry.test.ts`
...
- [x] Review the canvas sub-surface for the F5 pattern first — assertions that restate production, or `A || !A` shapes. It is the cheapest defect class to find and this file is where it will be densest.
      ...
      - **PAID 2026-09-16 by the reviewer pass — verdict: ZERO live F5 hits, and the sweep is now wider than the row asked.** Canvas sub-surface = 53 files (topolog*/canvasState/Inspector), three shape families grepped and every hit read; the excluded-middle family is still exactly one hit — the `topologyExport.test.ts:356` COMMENT documenting what `c442feecd` already deleted (a receipt, not a finding) — and the seven restate-shape candidates are all cross-consumer equivalence properties (two functions or two inputs each, every one falsifiable; `topologyKindRegistry.test.ts:206-211` actively ASSERTS the two-consumers-agree property F1 violates). Full table in `.agents/topology-canvas-review.md` §"Reviewer pass 2026-09-16". Recorded there too, because it will fool the next sweeper: PowerShell passes `topology*.test.ts` LITERALLY to rg — the first clean-looking 0 was a command that saw no files, silenced by `2>`$null.
- [x] Review the pointer, drag, keyboard and touch handlers for the F1 pattern — two consumers reading different sources of truth for the same scope.
      ...
      - **PAID 2026-09-16 by the reviewer pass — the table exists and the answer is NONE, structurally.** All six handler files (pointer 1,100ln, keyboard 591, drag-state 86, editor-touch 322, touch 43, bend-drag 207) read ZERO tenant-scope identifiers (`storeProfileId|store_id|sessionStore|branchLocation|selectedBranch|instanceId` → 0 hits each): the family is geometry-only, so the F1 shape has no habitat there. The scope-reading layer is tabulated consumer-by-consumer in `.agents/topology-canvas-review.md` (load lifecycle / apply / compare / history / revision-fetch / null-branch view) — every row keys on the same `selectedBranchId`, and the compare and history buttons' own render gates (`TopologyScreen.tsx` `stores.length >= 2 && selectedBranchId` / `canViewTopologyHistory && selectedBranchId`) prove the key is truthy wherever the consumers run. One SECOND-ORDER finding, recorded not fixed: `TopologyRevisionBrowser.test.tsx` snapshots baseRevision with NO branchId while deploying WITH one — it passes only because the mock's envelope is global (T-1), making it live evidence the open mock ruling has already taught a scaffold to rely on the wrong shape. If T-1 is ever fixed, that helper is part of the change.
...
- [x] Check `ui/src/dev-mock/handlers/topology*.ts`, which the review flagged as *"the third implementation of the same contract and … a plausible place for the next divergence"*.
      ...
      - **CHECKED 2026-09-16, by the devmock-router lane, WITH the concrete claim attached as the row demanded** — four findings T-1…T-4 in `.agents/topology-canvas-review.md` §"Follow-up 2026-09-16" (commit `4c6380204`; both mock files read in full at clean porcelain, backend compared by cited greps). The headline, one sentence: the mock's revision HISTORY is per-branch (`topology-state.ts:44/:73-78`) while its diagram ENVELOPE and revision counter are one global slice (`topology.ts:66/:79/:145`) — so editing branch A in browser preview moves branch B's canvas and burns a number B's history must skip, a state the real per-branch keying (`persistence.rs:101`, `commands.rs:69-84`) cannot produce. T-2 (opt-in revision gate) and T-3 (ignored requestId) measured UNREACHABLE by the live UI (`topologyApply.ts:144-145` always sends both) — recorded, deliberately not fixed; T-4 `can_save: true` is the global mock convention. Read-only fence obeyed: this row produced a finding, no code; the envelope fix, if the owner wants one, is three functions in `topology.ts` and belongs in a new phase, not this review box.
...
- [x] Do **not** fix anything found. Add the findings to this document as new phases, or write a follow-up `todo-` doc. A review that silently becomes a refactor is how a program loses its acceptance trail.
      ...
      - **HONORED 2026-09-16 — the fence held across the whole program, proven by commit type, not assertion.** The reviewer pass touched two paths, both documents: `d94bd313e docs(agents)` (the record) and this tick. Across the four ratified rulings and the review, no code commit followed any review finding — the residual that COULD have tempted one (the ANY-registry session-row acceptance under R4) shipped as a pinned test with an upgrade note instead. The prohibition did real work at the moment it was tested; kept as prose per the disposition, ticked now as the program's closing compliance record.
...
## 15. Closure — renamed to `done-todo-topology-editor.md` 2026-09-16

- **Boxes at the rename: zero open** (`rg -c '^\- \[ \]'` → none). All four owner rulings executed with red→green prints — R3/M5 `a718dd1e4`, R2/Phase 3 `4efcb0971`, R1/Phase 2 `8286f43ae`, R4/Phase 1 `32abb9cbb` — and Phase 7's reviewer rows closed by `d94bd313e` + `733d744e0`; the journal carries one entry per phase plus the program close (`2f9f17009`).
- **Renamed under the same §4 waiver its sibling lanes recorded** (KDS `273b0a455`, devmock-router `1b33b824a`): the whole-tree `check:all` remains red on the sales lane's ONE uncommitted CSS line — foreign, zero topology defects. Every order-attributable gate is green as of the last runs: `cargo test -p oz-pos-app --lib topology` **62/0/0**, `cargo test -p oz-bridge topology` **314/0/0**, `verify-ipc-parity.py` exit **0**, topology UI contract + consumer suites **145/145** and **63/63**, `npm run typecheck` **clean**.
- **Residuals live by name, not in this file's open-box list**: T-1 (the dev-mock's global diagram envelope, ruling still owed — with its dependent test-scaffold finding) in `.agents/topology-canvas-review.md`; the accepted ANY-registry ownership residual pinned by `session_only_row_authorizing_a_foreign_target_is_the_accepted_residual` with its upgrade note; the write-side self-seed it implies sits in the §I locations family, outside this program's fences.
- **Path-name citations age by design**: five code files still say `todo-topology-editor.md`, exactly as code still says `todo-payment-agents-*` after those lanes' renames — a dated record cites the name its date had. §14 already says what this program is not; this says when it stopped being open.
