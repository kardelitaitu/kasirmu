...
- [ ] `card` tender with `payment:edc` enabled → "Please tap/insert/swipe" overlay backed by `edc_sale_scoped` (long-running: surface its progress/timeout honestly; the mock fails closed per set_success — dev builds should demo well). → per (a)/(b): backed by real `edc_sale` after a scoped-status pre-flight; no cancel affordance during the blocking call — the terminal owns that moment, inventing a client-side cancel would be the dishonest surface.
...
- [ ] Optional pre-flight `test_edc_connection_scoped` — decide WITH agents-2's residual rather than duplicating it. → DECIDED in the re-audit above: pre-flight IS `edc_terminal_status_scoped`; no new command.
