...
- [x] `entryHandlers` empty or gone; `tauri-api.ts` reduced to: the vite-alias header, `export { convertFileSrc, invoke, isTauri }`, the `createXHandlers`/`registerHandlers` sequence, any documented residue, then the single `applyScopedAliases()`. **Target re-scoped by owner directive (2026-09-16, "we go with your recommendation"): the box measures CODE extracted / code-lines < 200, NOT raw `wc -l` < 200** — the literal line count is now dominated by deliberate documentation (the finding notes below explain why gutting it was not the right call). **MEASURED at HEAD `4b53fd215`:** total **368 = 119 code + 213 comment + 36 blank** → **119 code-lines < 200 = MET**, and the structural condition holds (`entryHandlers` reduced to its two imported spreads + 2 documented cross-domain residue stubs + `applyScopedAliases()`); zero direct `'key':` entries remain in the literal.
...
### Acceptance
- [x] **Before-snapshot registered-command set == after-snapshot set — MET (2026-09-16, final state HEAD `a2742d1ea`).** The dispatcher snapshot is **681 commands, sha256 `105d29730df2…60681c`, byte-identical** to the 5.0 baseline, re-derived through every one of the ~15 extraction moves (5.2/5.3/5.4, the 5.1 conversion-half, and all of 5.5 incl. the final `list_in_transit_transfers_scoped` re-home). No command dropped to the `invoke()` warn at any step.
- [x] **`npx tsc --noEmit -p tsconfig.json` → exit 0 — MET (re-run 2026-09-16 at HEAD `a2742d1ea`).** 0 errors on the fully-extracted router (368 lines).
- [x] **All `dev-mock-*` suites → exit 0 — MET (2026-09-16, HEAD `a2742d1ea`).** Ran all seven `dev-mock-*` suites together against a clean `ui/src/dev-mock/` tree (porcelain empty before/after → attributable to HEAD content): **7 files / 96 tests passed, `vitest exit 0`** — this closes the round-15 gap where the in-transit move was only spot-checked on a 4-file subset.
  > **Milestone (this session, 2026-09-16 05:49, HEAD `5748f8262`; dev-mock tip `93ed08fc5`).** The same
  > seven-suite green was first proven mid-refactor (96 tests, 681 identical, tsc 0) on a clean tree, then
  > re-confirmed at the FINAL state above. The three boxes this note sat under are the "did the refactor
  > break anything?" gates — all now ticked. They are NOT the whole-tree acceptance: `npm run check:all`
  > and the Phase 5.5 `< 200` line-count remain open below, so the file stays `todo-` (see the two
  > following boxes and the §4 reminder).
...
> **RAN 2026-09-16 ~09:15 (this lane, owner "try again") at HEAD `470e372eb` — `npm run check:all` → exit 1, 7 passed / 2 failed. Every code leg green; both reds environmental or foreign:**
> - vitest FAIL was **one stale assertion in a committed file**, not foreign WIP: `NodeTopologyEditor.test.tsx:8028` still asserted `mockLoadTopology` called with `('branch-a')` after `8286f43ae` (topology Phase 2, R1) sessioned the read. The harness renders with `MOCK_SESSION_TOKEN`, so the measured call is `('mock-session-token', 'branch-a')`. Fixed in `470e372eb test(ui): update the topology load assertion for the sessioned read (R1)` (+3/-1, pathspec, this file's fence untouched) — isolated run green. Note the mechanism: the topology lane updated its contract tests but missed this viewport-memory case in the same commit.
...
>
> - **Re-run 2026-09-16 (this session, owner "do all") — the "upgrade to a real green" attempt. Result: `check:all` still not exit 0, and NOT because of the router.** The foreign `CartPanelLineItem.css --shadow-md` red had **self-cleared** (a solo `themeTokenCompliance` run is now 52/52 green), so I re-ran the whole gate with Git `bash` on PATH to neutralize the i18n WSL trap: **i18n / FTL / bundle / bundle-tablet / ESLint / perf all PASS**, but **typecheck FAIL**, **vitest FAIL**, **E2E FAIL**. Attribution, none of it this order: typecheck fails on foreign uncommitted `CartPanel.test.tsx`; vitest fails as a PosScreen-cluster mass-red (foreign WIP), and one collateral hit landed on `dev-mock-auth-contract.test.ts` — which I proved is **not** a router regression: `ui/src/dev-mock/` was clean and that suite passes **26/26 in isolation (exit 0)**; it only fails when the shared full-suite process runs the broken PosScreen localStorage tests alongside it. E2E is the stale-Docker-image guard ("0 tests executed, not a test failure"). So the router's `check:all` box **legitimately stays unticked** — not for any dev-mock defect, but because the shared checkout currently carries foreign WIP that does not even typecheck. **The §4 waiver remains the accurate close, and this run strengthens it rather than replacing it.**
