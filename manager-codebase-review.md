...
The most useful thing in this section is that the repository already knows, precisely, and writes it down. Both shells generate a registration-gate ledger with a debt ceiling: desktop registers **DEBT_CEILING = 74** (47 no_session_resolution plus 27 resolves_session_names_no_permission) and the tablet **94** (50 plus 44) (registration_gate_debt.generated.rs:214, :223, :226 and :325, :354, :361). Those are not sampled estimates; they are the runtime registration table.
...
Do **not** add FORCE ROW LEVEL SECURITY to the generated migration while the application still connects as the table owner - every query would return zero rows on deploy. The safe sequence is a role/ops change: create the oz_app role, grant DML, switch DATABASE_URL, verify, and only then consider FORCE as a follow-up that re-asserts what ops already did. Webhooks need their BYPASSRLS path preserved (rls-cutover.sql:33-42), and the cutover script's 'all 19 tables' comment is stale against its own 34-table array. Keep the reversal (NO FORCE plus DROP ROLE) documented and rehearsed.
Acceptance: a tenant-table query without the tenant GUC returns zero rows and a write is rejected; a query with the GUC returns only that tenant's rows.

**P0-7. Default to a real at-rest key.**
If OZ_MASTER_KEY is not set on a deployment, everything this product calls encrypted is derivable from the repository. Either require it at boot on any build that persists credentials, or adopt the keychain-backed key that was deliberately deferred - and correct the README's 'platform keychain' clause so it describes the two mechanisms separately.
Acceptance: a fresh install without an explicit master key either refuses to start or generates and keychains one; a test asserts that the static fallback cannot be reached in a release build.
