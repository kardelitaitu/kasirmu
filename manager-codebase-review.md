...
The casualty is not cosmetic: the tablet device-code pairing flow that ADR-56 section 2.5 exists for is dead in the shipped image, and the Midtrans snap and webhook endpoints never reach their handlers. The underlying defect is a process gap, not a typo - the Caddy carve-out list was never reconciled against the license server's route table. That gap has since produced a checker, and the checker is red: scripts/check-unified-routes.mjs exists, is registered as a required gate (scripts/check.sh:102, gates.json:613), and currently exits 1 on this tree with 'unified-routes: 1 route prefix(es) reach the Rust service by default - /api/v1/pairing/* is not carved out'. Re-run it to confirm; the pairing outage is a failing gate nobody has cleared, not an inference of this review. It also has a structural blind spot: it derives prefixes from string literals in main.go alone, while the Midtrans paths live in constants in other files, so it cannot ever detect the second missing namespace. The same file also defaults /api/* to PocketBase (:106-109), so any future sync route registered under /api/ silently changes owner.
