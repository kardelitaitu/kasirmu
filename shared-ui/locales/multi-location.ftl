# shared-ui/locales/multi-store.ftl — Multi-store dashboard
...
topology-apply-confirm-note-label = What changed?
topology-apply-confirm-note-optional = optional
topology-apply-confirm-note-placeholder = e.g. Opened the Pos Kota counter; moved Gudang to back-room routing
...
topology-apply-confirm-pin-remember = Remember PIN for this session
...
topology-history-open = Deploy history
# ── ADR #46: deploy history browser ────────────────────────────
...
topology-rev-browser-pin = Pin this deploy
topology-rev-browser-unpin = Unpin this deploy
topology-rev-browser-detail-title = Revision { $revision }
topology-rev-browser-deflated-body = This deploy is recorded but its graph was pruned by retention, so it cannot be previewed or restored. Published by { $by }.
topology-rev-browser-not-found = This revision is no longer present.
...
topology-apply-permission-tooltip = Applying topology changes requires the topology:write permission.
topology-readonly-note = View-only — saving topology changes requires the topology:write permission.
topology-rename-permission-error = Renaming topology items requires the topology:write permission.
...
topology-empty-state-body = Click a tool in the palette (or press 1–4 with the canvas focused) to add a node. Connect nodes with the port sockets on each card.
...
topology-rack-delete-empty = Select a node or wire to delete
topology-export-desc = Download as JSON
topology-import-desc = Load from JSON file
...
topology-action-active = Active
topology-action-disabled = Disabled
topology-action-properties = Properties
topology-action-disconnect = Disconnect
...
topology-inspector-timezone-placeholder = Select a timezone…
