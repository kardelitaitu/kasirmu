...
· **`:189` (optimistic → enqueue → reconcile)** and **`:191` (never roll back a durable item)** — `ls ui/src/__tests__ | grep -i offline` → 10 files including `api-offline-contract.test.ts`, `KdsOfflineRearmDeadLetters.test.ts` and `KdsOfflineReadWriteLS.test.ts`, and `grep -oE 'syncedCount|failedCount|conflictCount' ui/src/api/offline.ts` → 3/3/3, so both the reconcile shape and the dead-letter path already have a graded home.
