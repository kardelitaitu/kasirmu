...
- **"27 OZ-POS crates" is now 35.** `cargo metadata --no-deps` lists 35 workspace members, all with `license = "SEE LICENSE IN LICENSE"`, and `deny.toml` carries 36 `[[licenses.clarify]]` entries. Re-measure: `cargo metadata --no-deps --format-version 1`.
