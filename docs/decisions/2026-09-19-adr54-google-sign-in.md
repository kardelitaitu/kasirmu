...
> up instead of leaving a stale secret in a running process. That file also *writes* them on its own
> pairing path, so the two are two ways to the same state rather than a
