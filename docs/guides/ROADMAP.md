...
- [ ] ~~Prometheus metrics endpoint (optional, in `oz-reporting` behind `metrics` feature — counters, gauges, histograms + HTTP server in `platform-startup`)~~ **PARTLY FALSE — corrected 2026-09-12: the counters half is real, the "+ HTTP server in `platform-startup`" half has never compiled.** Unwired, not broken. What ships: `oz-reporting` really does declare the feature (`metrics = ["dep:prometheus"]` in `crates/oz-reporting/Cargo.toml`) and its counters, gauges and histograms sit behind it in `crates/oz-reporting/src/lib.rs`. What does not: `platform/startup/src/metrics.rs` wraps its entire `pub mod server` in `#[cfg(feature = "metrics")]`, but `metrics` appears nowhere in `platform/startup/Cargo.toml` — that crate declares only `default`, `console` and `whatsapp-notifications` — so the cfg names an undeclared feature and is false in every build, including CI's `--all-features`. Narrow that precisely, because it matters: `platform/startup/src/lib.rs` declares `pub mod metrics;` unconditionally, so the file itself — its doc comments and its outer items — is read and checked by every build; what never compiles is the inner `pub mod server` that the gate encloses, and that inner module holds the whole endpoint, `start_metrics_server` included. Nothing ever warned about this, and the reason is in the source: the first three lines of `platform/startup/src/lib.rs` are a two-line comment and `#![allow(unexpected_cfgs)]`, the comment justifying the allow by saying it permits `cfg(feature = "metrics")` "from the transitive dependency on `oz-reporting`" without requiring `platform-startup` to declare the feature. That premise is false — a dependency's features do not enter the cfg space of the crate that depends on it — and `unexpected_cfgs` is exactly the rustc lint that would have named the undeclared feature. So this has never been a compile warning somebody chose to ignore; it is a lint switched off by a comment written on a misunderstanding. The code is honest about its own condition (the module doc reads "When the `metrics` feature is enabled…") and the comment is the record of the choice, so naming it is not an accusation. But the allow is load-bearing for the silence: taking it out before this box is decided would flood the build with a warning nobody owns, so the feature question has to be answered first and the attribute comes out in the same change. Its body also calls `oz_reporting::gather_metrics()` while the crate's only reporting dependency is `modules-reporting`, so declaring the feature alone would still not build. Proof that survives someone wiring it up tomorrow: `start_metrics_server` has no call site in either Tauri client or in `apps/cloud-server`. **What is actually true about metrics today:** the desktop and tablet apps expose no metrics endpoint at all, so a reader looking for `/metrics` in the app will not find one; the only `/metrics` route in the tree is the cloud server's own (`metrics_handler` in `apps/cloud-server/src/main.rs`, rendering `apps/cloud-server/src/metrics.rs`'s separate registry — not `oz-reporting`'s, and the cloud server does not depend on `oz-reporting`), and what a hosted deployment actually serves is not checked by this line. The nearest prose concession, "Prometheus counters exist but are not exported" (`docs/decisions/2026-09-02-adr43-cloud-sync-performance-scaleout-roadmap.md`), is about the hosted stack's observability pipeline, not this box — quoted here so a reader does not mistake it for evidence that the app-side server exists. Both endings are legitimate and only one of them is currently claimed: either bring the box down to what ships (feature-gated counters, no app-side server), or declare the feature, add the `oz-reporting` dependency and start the server from a client `setup`.
