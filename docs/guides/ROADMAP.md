...
- [x] File writer with rotation (`kasirmu_logging::init_with_file()`, `kasirmu_logging::init_json_with_file()`)
    ...
    - **Implemented, never wired — corrected 2026-09-12.** The two sentences
      ...
      (`apps/desktop-client/src/lib.rs`, `apps/mobile-tauri/src/lib.rs`), which
...
- [x] Scheduled report delivery (email PDF) — shipped in 0.0.22 (`apps/desktop-client/src/email_scheduler.rs` + `kasirmu-core` email export)
