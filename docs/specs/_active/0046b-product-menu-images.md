...
<!-- Audit stamp: 2026-09-09 · DSH · status: ACCURATE AFTER REPAIR (1 finding) · §2 is titled "Ground truth in the codebase today", so its rows assert present tense and are repaired rather than annotated — this is not an intended-future-state claim. Fixed row: the prod-topology row cited `.github/workflows/deploy.yml` as evidence of how `oz-cloud` reaches production; that file has been inert `deploy.yml.bak` since `23c963303` (2026-09-02, a rename of the whole set, `name: Deploy Backend (Northflank)` at `deploy.yml.bak:46`), and the live trigger is `dev-ci.yml#northflank-deploy` — verified in the workflow itself at `.github/workflows/dev-ci.yml:653` (job), `:671` (`if:`), `:675` (`NF_SERVICE_ID` default `oz-pos-cloud`) and `:697` (POST .../services/$NF_SERVICE_ID/build). The rest of that row's evidence was re-checked against the source, not the doc: `ops/docker/Dockerfile.unified:179-215` does show the single `/data` volume (`VOLUME ["/data"]` at :202, `OZ_DB_PATH=/data/oz-pos.db` at :204), the three binaries copied in (:190-192) and `CMD ["supervisord", ...]` (:215), and `apps/unified/supervisord.conf` is tracked · NOTE for a reader of this spec: `northflank-deploy` fires on `workflow_dispatch` only in practice — `dev-ci.yml` has no push trigger, so "deployed by CI" means a human pressed Run workflow · LEFT ALONE: every other §2 row (they name files that exist and were not re-derived in this pass) and all forward-looking design sections, which are proposals, not CI claims. -->
...
| Cloud byte store = Northflank volume (`VOLUME ["/data"]`, `OZ_DB_PATH=/data/oz-pos.db`) | `ops/docker/Dockerfile.server`, `docs/plans/northflank-p1-p7-plan.md` |
...
| Prod service `oz-cloud` = one combined container: **supervisord {caddy, license/PocketBase, sync/oz-api}** on the single `/data` volume; Caddy terminates auto-TLS with HTTP/2+3 | `ops/docker/Dockerfile.unified:179-215`, `apps/unified/supervisord.conf`; what deploys it is `dev-ci.yml#northflank-deploy` (`.github/workflows/dev-ci.yml:653`, service id defaults to `oz-pos-cloud` at `:675`, build POST at `:697`) — this row cited `.github/workflows/deploy.yml` until 09-09-26, and that workflow is inert (`deploy.yml.bak`, renamed by `23c963303` on 2026-09-02), so it proved nothing about what runs |
| Migrations are a versioned `Migration` list applied into `schema_migrations` (incremental, idempotent); the PG schema is regenerated by script | `platform/core/src/database/migrations.rs:44-56`, `crates/oz-core/src/migrations.rs:33-41`, `scripts/generate-pg-migration.py` |
...
- **Data model (both migrations, sqlite + pg):**
  ...
  - Assignments ride the product's existing `version` — any image-set change
    bumps `products.version`, so the catalog snapshot serves the row + its
    image rows atomically per product.
...
3. **Transcode & resize:** decode via the `image` crate (webp feature —
   lossy WebP *decoding* is supported; animated inputs take frame 1; HEIC/
   AVIF are rejected with a typed error) → EXIF orientation applied → EXIF
   stripped → resize to **512 px longest edge** (decided — headroom for
   2x-DPR tablet tiles) → encode with the **`webp` crate (libwebp)**.
   Second-mechanism audit note: the `image` crate's own WebP encoder is
   **lossless-only** — encoding q40 without libwebp is impossible, and that
   would have surfaced as a P1 blocker. libwebp-sys compiles under cargo-ndk
   (cc toolchain), feature-gated to the two app binaries.
   **Adaptive quality targeting:** encode q40 first; if the result exceeds
   32 KB, retry q30, then q24 (floor); hard-reject > 48 KB with a typed
   error. Noisy photos (glitter, foliage) blow past 32 KB at q40 — the
   targeting loop keeps the per-file caps honest (target ≈ 10–25 KB).
   Decoded ARGB ≈ **1 MB** per image in RAM — which is exactly why §3.5
...
5. **Assign in one transaction (repo rule):** upsert
   ...
   `products.version`. `products_clear_image(product_id, slot)` mirrors it. Menu edges: creating
   a menu-typed item **requires the initial bytes** (upload-at-apply applies
   at creation too — a menu item never exists without an image); retyping a
   product with > 1 image to menu is refused with a typed error (the UI
   trims alternatives first).
...
- **Transport truth (audit-verified):** catalog authoring is REST writes, and
  tablets receive the catalog via **REST snapshots** — so the hashes ride the
  snapshot: `list_products` responses gain an
  `images: [{slot, hash, position}]` array per product (products do not ride
  the offline queue — verified: the queue carries sale.* actions only).
  The byte store never participates in conflict logic (immutable
  content-addressed files — the 0045 dead-letter path stays untouched).
...
- oz-api serves the bytes itself — **no new component**:
  - `PUT /api/v1/images` — the upload primitive (the batch lane in §3.6
    ...
    *actual* catalog tier — bare JWT, matching `create_product` — NOT
    ...
    `{hash16}`.
  - `GET /api/v1/images/{hash16}` (tenant JWT **plus an
    `image_refs(tenant_id, hash)` existence check** — one indexed lookup
    closes cross-tenant fetch, which unguessable-hash security alone does
    not; strict hash-grammar validation kills directory traversal; immutable,
    `Cache-Control: max-age=31536000, immutable`; unknown hash ⇒ 404).
  - Natural dedupe: re-uploading an existing hash is a no-op success — and
    the `image_refs` refcount still increments on `duplicate` (audit fix: a
    no-op that skips the refcount would let GC eat a file another product
    just started referencing).
...
- Tablet pull: a tiny **download manager** in the tablet's Rust shell — on
  ...
  configurable budget, default **256 MB** — audit fix: 64 MB is smaller than
  the primaries alone of a 5k-product catalog (~100 MB), which would thrash
  the store with re-downloads; 256 MB ≈ ~12k images ≈ a 5k catalog with
  primaries + ~40% of alternatives). Eviction is **slot-aware**: alternatives
  evict before primaries, so POS tiles never thrash. Offline-first holds: missing image degrades to the
...
sync (§3.4 — REST snapshot + writes): byte transfers are the expensive ops,
so they must never delay catalog sync or hold the DB lock. Both lanes run inside the existing
...
- Failure backoff (precise, AWS full-jitter):
  `delay = uniform(0, min(30 min, 60 s · 2^attempts))` — attempt 1 sleeps
  0–60 s, attempt 3 up to 4 min, attempt 5+ up to the 30 min cap; after 8
  attempts the entry dead-letters (0045 pattern) and the desktop UI surfaces
  "image pending upload" on the product row.
- Server-driven nudge: `image_refs` tells the server exactly which hashes
  product rows reference and which files exist — `list_products` responses
  append `missing_hashes` so the desktop prioritizes exactly what the cloud
  lacks in its next push batch (no polling, no full-scan).
...
**Server-load accounting (honest numbers, audit-corrected):** steady state
(nothing missing, nothing queued) is ≈ **0.6 req/s** at 100 devices — pull
cycles with an empty missing-set issue no GETs, push cycles with an empty
queue issue no POSTs. The real burst is post-update: a menu refresh making
40 images missing on every tablet ⇒ 100 × 40 GETs across a ~4-min
jitter-spread window ≈ **17 GETs/s for a few minutes** of immutable
static-file serving — well inside axum + Caddy headroom, with the per-device
phase seeding flattening the spike. Constants
...
2. **Exact server-side content spine — `image_refs`.** The cloud keeps
   (tenant_id, hash, refcount, bytes), maintained transactionally when the
   catalog REST writes apply (audit-corrected: catalog flows over REST
   snapshots and writes, not deltas). Four features fall out of one table:
   `missing_hashes` on the snapshot response becomes an exact SQL
