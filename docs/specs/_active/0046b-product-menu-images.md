...
| Prod service `oz-cloud` = one combined container: **supervisord {caddy, license/PocketBase, sync/oz-api}** on the single `/data` volume; Caddy terminates auto-TLS with HTTP/2+3 | `Dockerfile.unified:179-215`, `apps/unified/supervisord.conf`; what deploys it is `dev-ci.yml#northflank-deploy` (`.github/workflows/dev-ci.yml:653`, service id defaults to `oz-pos-cloud` at `:675`, build POST at `:697`) — this row cited `.github/workflows/deploy.yml` until 09-09-26, and that workflow is inert (`deploy.yml.bak`, renamed by `23c963303` on 2026-09-02), so it proved nothing about what runs |
