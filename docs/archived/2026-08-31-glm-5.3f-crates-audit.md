---
area: architecture
title: kasir.mu Full Crate Audit — GLM 5.3-Flash (RSA)
status: Archived (2026-08-31) — campaign fully closed (sections 1–46), no open findings
---

> **Status:** Archived (2026-08-31) — campaign fully closed.
>
> Sections 1–46 closed: every one of the 32 original targets plus `website/`,
> `apps/license-server`, and the build/release/CI supply chain audited, all approved
> findings fixed and committed, no open items and no deferred follow-ups (see the §46
> tally at the end of this file). Kept as a historical audit record.
> Companion app-level review:
> [`2026-08-30-glm-5.3-tauri-app-review.md`](./2026-08-30-glm-5.3-tauri-app-review.md).
>
> Note: this document was assembled by appending campaign slices, so its own title
> heading and severity legend sit below the opening slices and the section numbering
> is not monotonic. Left as written.
...
# kasir.mu Full Crate Audit — GLM 5.3-Flash (RSA)
...
### Slice C — AccountView.tsx (516 lines after the ./account/* split) — 31-08-26, CLEAN

Fully read (the earlier 1,006-line monolith was split into the stateless presentational sections in `./account/*`; this file owns state + data flow). **No findings requiring code changes.** Verified architecture: loading/anon/error/ready state machine; session resolves cookie-first (`/__oz/session` httpOnly) with sessionStorage fallback, 401 → `clearSession` + anon; every async `setState` is `mountedRef`-guarded; checkout polling is bounded (8 × 2.5 s) with an explicit "webhook still provisioning" state; device revoke optimistically stamps the revoked row without ever collapsing the list to the fallback hint; region drives payment routing (ADR #39: id → Midtrans, else Paddle) identically to the pricing page; placeholder Paddle price ids gate both subscribe and bundle sections (honest WIP comments in-code); device id is `encodeURIComponent`-escaped; all mutations authenticate via Bearer header (not cookies), so there is no meaningful CSRF surface.

ℹ️ INFO (no action): the Worker's `GET /__oz/logout` is a logout-CSRF nuisance vector (conventionally accepted for logout endpoints); client-side password-strength checks are delegated to the server's `set-password` gate (correct layering); the placeholder-price WIP is documented in-code and tracked by subscription-tiers.md.

**This closes section 44 (website/) entirely — all slices A–C stamped.**
...
### Slice D (part 1) — admin gates: admin_dashboard.go (396), addon_admin.go (235), enterprise_admin.go (173) — LSE-8/LSE-9, fixed 30-08-26

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-9 | 🔴 HIGH ✅ FIXED 30-08-26 (`41300ff1`) | addon_admin.go:223–235 (`authenticateAdmin`), enterprise_admin.go (both handlers' inline auth) | The admin-only endpoints **accepted ANY valid tenant api_key** — the enterprise_admin.go doc comment admitted it verbatim ("For now, any valid tenant API key is accepted"). Any activated customer could: **mint enterprise trial approval codes** (`POST /api/v1/admin/enterprise-codes`, custom code of their choosing → free enterprise trials via the redeem endpoint), **list every approval code** incl. other prospects' unused codes + emails, and **add/remove arbitrary add-ons on any license key** (self-upgrade; today record-tampering since add-ons aren't yet consumed by the signed payload, but the C4.3 marketplace will consume them). No rate limiting either. | Fixed: `authenticateAdmin` now mirrors `adminAuth` — OZ_ADMIN_KEY bearer or an admin-tenant web session (OZ_ADMIN_EMAIL → defaultAdminEmail), pure boolean; enterprise handlers deduplicated onto it; doc comment corrected. Regression tests prove a valid non-admin tenant key is rejected on both endpoint families (`TestAddLicenseAddon_TenantKeyRejected`, `TestGenerateCode_TenantKeyRejected`). Test infra: `seedAdminTenant` arms `OZ_ADMIN_KEY` (t.Setenv, auto-restored) so all 17 existing scenarios pass unchanged against the tightened gate. |
| LSE-8 | 🟡 MED ✅ FIXED 30-08-26 (`41300ff1`) | admin_dashboard.go:34–42 (`adminKeyOK`) | The admin bearer secret was compared with plain `==` — the **only non-constant-time secret comparison** in the service (api_keys, OTP codes, webhook signatures all use `subtle`). | Fixed: `subtle.ConstantTimeCompare` with a length pre-check. |
| LSE-10 | ℹ️ INFO | admin_dashboard.go (adminAuth session path) | The admin-tenant session fallback checks only the email — a **suspended/revoked admin tenant** would keep dashboard access until the session expires (≤24 h + touch). | Accepted (deliberate anti-lockout posture for the operator). If tightened: also require `status == "active"`. |

Also noted: `admin_dashboard.go` was concurrently fixed by the parallel session mid-review (the stale `version: "0.0.31"` health literal now `adminDashboardVersion = "0.0.33"` — their B31 note); my LSE-8 edit landed cleanly on top. Minor perf note recorded: `handleAdminListTenants` loads all tenants to count before the paginated query — fine at current scale, worth a count-query if the tenant table grows.

**Verified:** `go build` + `go vet` clean; full `go test ./...` green (136 s) including the two new LSE-9 regression tests.

### Slice D (part 2) — activate.go (846 lines, fully read) — 30-08-26

**Verdict: well-engineered.** Verified: consistent tenant→key lock ordering with re-reads under lock (no TOCTOU on key status or machine counts); Bearer-only credential channel with an explicit rejection + hint for the legacy body field; per-IP bucket (5/hr) + per-key escalating cooldown, recorded on every failure path and cleared on success; no-enumeration on key states (activated-by-other / revoked / unknown all answer the same generic 401); machine limits enforced on BOTH paths (re-activation uses the *active subscription's* tier so downgrades bite); the hardware-fingerprint trial lock fires at mint time so skipping `/trial` cannot bypass it; segmented `trial_vertical`/`bundle_id` are honored **only for `is_trial` keys** — a forged client parameter can never shorten or downgrade a paid license; webhook-issued keys mint a real api_key (the webhook's placeholder hash is unusable by design); api_key plaintext exists only at the mint/rotation instant.

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-11 | 🟡 MED ✅ FIXED 30-08-26 (phase B `4984095d`, phase A `2635091e`, docs `3c23e47b`) — b-then-a per owner decision | activate.go:482–507 (re-activation api_key rotation) | Anyone who knows a tenant's **email + full license key string** could force an api_key rotation and receive the new credential: the re-activation path accepts email+key as sufficient proof (deliberate, for reinstalls that lost the api_key), and when `verifyAPIKey` fails it rotates the tenant's api_key and returns the plaintext to the caller. Impact: silent takeover of the tenant's api-key-gated surface (`/renew`, `/status`) + victim lockout ping-pong (each re-activation rotates again; successful activations clear the per-key cooldown). The attacker's machine also registers on the victim's tenant (Free tier's 1 slot → "machine limit reached" for the victim's real machine). No financial minting — the signed subscription is untouched. | **Phase B (`4984095d`):** `apiRotationLimiter` — at most one rotation per tenant per 24h (further attempts → 429 with `retry_after`, no re-emit); every successful rotation emails the owner (best-effort SMTP notice); `windowLimiter.remainingSeconds` helper. **Phase A (`2635091e`):** rotation now requires inbox proof — new `POST /api/v1/license/recover {email, key}` proves the same email+key ownership as activation (generic 401, per-key escalating cooldown, recoverLimiter 3/15min/email + shared 5/hr IP bucket, both before lookups) and emails a 6-digit code (web OTP store: hashed, single-use); re-activation without a valid api_key returns the subscription + `api_key_rotation {status: recovery_required}` — no rotation; with `recovery_code`: constant-time verify → rotate + notify (24h cap + notice email still apply); wrong code → 401, nothing rotated. **Docs (`3c23e47b`):** website getting-started (`welcome.md` en+id "Reinstalling or locked out?") + full recovery guide (`activation.md` en+id). Tests: full cycle, cooldown, bad-key, wrong-code; lifecycle updated. |
| LSE-12 | ℹ️ INFO (documented design) | activate.go:109–123 (`enforceTrialLock`) | The fingerprint trial lock is **advisory for legacy clients**: a client that omits `hardware_fingerprint` and sends a non-fingerprint-form `machine_id` skips the lock entirely (`normalizeHardwareFingerprint` rejects non-`hw_` forms rather than bricking activation). A determined trial-abuser with fresh emails can evade the per-device lock; remaining bounds are trial-key availability, the per-IP activation budget (5/hr), and the repeat-email detector. | Accepted and documented in-code. If trial abuse materializes: require a fingerprint form for trial keys specifically (paid keys unaffected), minting trial activation impossible without `hw_`. |

### Slice D (part 3) — renew.go (252), status.go (170), trial.go (272) — 31-08-26

**status.go: clean** (Bearer-only, generic 401s, pre-auth shared IP bucket, tenant-scoped machine lookup, P8-2 revoke properly scoped).

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-13 | 🟡 MED ✅ FIXED 31-08-26 (`3695bb31`) | trial.go (handleTrial) | **No rate limit on `POST /api/v1/license/trial`** — fingerprints are client-derived, so unlimited unauthenticated writes: each unique fingerprint inserts a `trial_registrations` row, and a supplied email also **upserts a tenants row** (unauthenticated tenant creation). The claim itself mints no subscription (that happens at the rate-limited activation), but the endpoint is an unbounded DB-fill vector. | Fixed: shares the persisted per-IP token bucket (5/hr) with activate/renew/status, applied after body validation (mirrors activate.go ordering). Test: exhausted budget → 429, nothing persisted. |
| LSE-14 | 🟢 LOW ✅ FIXED 31-08-26 (`3695bb31`) | renew.go:199–230 (renewal write ordering) | The old subscription was marked `expired` **before** the replacement was saved — a failed new-sub save mid-renewal left the tenant with **zero** active subscriptions (the POS keeps running on its last signed payload, but renewal state was destroyed). | Fixed: save the new subscription first, then expire the old. Worst case on a later failure is two active subs, which activate/renew/status already resolve by `-starts_at`. All 12 renew tests green. |
| — | ℹ️ INFO | renew.go:158 | `case "free": newExpiresAt = baseTime.AddDate(100, 0, 0)` — renewing with a "free"-tier key extends expiry 100 years. Harmless self-downgrade (free keys shouldn't be purchasable/minted), but the magic number is a smell. | If free keys can never legitimately exist in `license_keys`, reject renewal keys with `tier_key == "free"` instead. |

renew.go otherwise verified: Bearer-only auth, payment-gated (requires an unused purchased key), tenant+key locks in consistent order, M5 quota fixes (new key's quotas), signed payload matches the DB row, key expiry checked before use, failure tracking on key mismatches only.

> **Provenance (race #5, `3695bb31`):** the commit also carried two UI files owned by the parallel session (`ui/src/features/stores/NodeTopologyEditor.tsx`, `topologyToolRack.tsx`) under my `fix(licensing)` subject. Verified: my three files intact in HEAD, `npm run typecheck` clean on the swept code. Recorded per convention.

### Slice D (part 4) — pause.go (87), resume.go (70), expiry.go (56) — 31-08-26

**expiry.go: clean** (pure helpers — tier durations, grace +14d, machine quotas).

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-16 | 🟡 MED ✅ FIXED 31-08-26 (`fd347b62`) | pause.go, resume.go | **The only unthrottled `findTenantByAPIKey` (bcrypt) endpoints** — unauthenticated clients could hammer them for cheap CPU exhaustion (the exact flaw status.go's limiter comment already describes; every request pays a ~60–100 ms bcrypt verify). Also no 64KB body caps. | Fixed: both share the persisted per-IP bucket (5/hr) — pause after body validation (activate.go ordering), resume before auth (status.go ordering); bodies capped. Test pins 429 for both on an exhausted IP. |
| LSE-15 | 🟡 MED ✅ FIXED 31-08-26 (`f929fc75`) | resume.go:51–54 | **Resume never extended `expires_at`** — the doc comment claimed "resets the billing cycle from now" but the code only flipped status and cleared the pause fields. A customer who pauses 3 months and resumes loses those paid months. Worse: there is **no auto-resume scheduler anywhere** (`paused_until` is only honored lazily when a resume call arrives), so a "paused" subscription that nobody resumes just sits expired-looking while the device's signed payload (still `active`, original expiry) keeps working offline. | Fixed (extend-and-re-sign, renew semantics): resume extends `expires_at` + `grace_until` by the time actually paused (capped at the planned pause end) and re-signs the payload from the subscription row; the response returns the fresh `signed_payload`/`signature` so the device syncs on the same call. New daily auto-resume scanner in main.go resumes subs past their pause window (planned length credited). Tests: early resume (+10d exactly, signature renewed) and scanner (100d elapsed / 90d planned → 90d credited). |
| — | ℹ️ INFO (offline-first inherent) | pause.go | Pausing cannot actually stop a POS that doesn't call home: the device holds a signed payload valid to the original expiry, verified offline. Pause is advisory (visible via /status) — same trust model as revocation vs grace periods. | Documented; no action. |

### Slice D (part 5) — enterprise_trial.go (206), enterprise_admin.go (229) — 31-08-26

The redeem path (public by design, gated by sales-minted approval codes): B45's mutex makes one-time codes single-redemption even under concurrency (reproduced race → fixed); tenant creation happens only AFTER a valid code; minted keys are `is_trial` so activation enforces the device trial lock.

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-17 | 🔴 HIGH ✅ FIXED 31-08-26 (`1fd98b27`) | enterprise_trial.go (`generateEnterpriseTrialKey`) | **Trial keys were generated from clock jitter, not a CSPRNG**: every character was `time.Now().UnixNano()%32` with `time.Sleep(1)` between characters. Those keys mint 30-day **enterprise** trials — a predictable keyspace let an attacker mint trials without any approval code by predicting keys minted near a known redemption (the sleeps make the increment pattern regular). | Fixed: characters from `crypto/rand.Read` (log.Fatalf on failure, same policy as `generateAPIKey`); the 32-char charset divides 256 so `byte%32` is bias-free. Regression test: 200 rapid keys, exact-format pin, all unique. |
| LSE-18 | 🟡 MED ✅ FIXED 31-08-26 (`1fd98b27`) | enterprise_trial.go (handleEnterpriseTrial) | The redeem endpoint was unthrottled — unlimited probing of the approval-code space; the 403 (invalid) vs 409 (already redeemed) distinction even maps spent codes. Tenant creation is correctly gated behind a valid code, so the exposure is probing + DB lookups. | Fixed: shares the persisted per-IP bucket (5/hr) + 16KB body cap. |
| LSE-19 | 🟢 LOW ✅ FIXED 31-08-26 (`1fd98b27`) | enterprise_admin.go (`generateApprovalCode`) | Ignored the `crypto/rand` error — on a broken entropy source it would deterministically mint `ENT-00000000`. | Fixed: fatal on failure, matching `generateAPIKey`. |
| — | ℹ️ INFO | enterprise_trial.go:112–141 | Redeem creates new tenants with an api_key whose plaintext is **never returned** — a new enterprise trialist must use the LSE-11 phase A recovery flow (email code) to obtain a management key before they can renew/status. Consistent with the trust model, mildly awkward UX. | Documented; consider emailing the api_key only over the same recovery-proof channel. |

### Slice D (part 6) — contact.go (213), smtp_mail.go (300+), health.go (218) — 31-08-26

**health.go: reasonable** (gates are status-not-liveness so monitors don't flap the container; SMTP probe cached 60s; DB-only liveness). ℹ️ INFO (no change): the public payload exposes `go_version`/`go_os`/`go_arch`/raw `db_error` — a small fingerprinting surface, conventional for health endpoints; `db_error` could leak internal paths on a degraded DB. ℹ️ INFO: no rate limit on /api/health — one trivial `SELECT 1` per call, standard practice for Docker healthchecks.

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-21 | 🟡 MED ✅ FIXED 31-08-26 (`53ae8936`) | smtp_mail.go (non-465 send + probe) | **STARTTLS was opportunistic**: `smtp.SendMail` upgrades only when the relay advertises it and silently continues in plaintext otherwise — leaking auth credentials and message bodies (OTP codes) to a relay that skips, or a MITM that strips, the advertisement. The boot probe skipped equally silently. | Fixed: the non-465 path REQUIRES STARTTLS (explicit operator-facing error → use port 465); probe enforces the same; both TLS configs pin MinVersion TLS 1.2. Test pins the refusal. |
| LSE-22 | 🟡 MED ✅ FIXED 31-08-26 (`53ae8936`) | smtp_mail.go (send path) | **No timeout on the actual send path** — a relay that accepts the connection but stalls pinned a request goroutine forever (OTP emails are sent inside request handlers). Only the boot probe was bounded. | Fixed: both transports bounded end-to-end by a 15s deadline (DialWithDialer + SetDeadline), unified in one `deliverSMTPMessage` which also refuses to silently send unauthenticated when credentials are configured but the relay doesn't advertise AUTH (previously skipped). |
| LSE-20 | 🟢 LOW ✅ FIXED 31-08-26 (`53ae8936`) | contact.go (postToDiscord) | `name`/`email` from the unauthenticated form were interpolated as Discord markdown **outside** the code fence — `@everyone` in the name field pinged the whole support channel (webhook mentions are on by default). Also: `message[:budget]` byte-slicing could split a multi-byte rune into invalid UTF-8. | Fixed: payload sets `allowed_mentions {parse: []}` (mentions off) + rune-safe truncation. |

contact.go otherwise verified: dedicated in-memory limiter (5/hr, before validation, deliberately not shared so spammers can't drain the activation budget — well-reasoned), honeypot, strict field validation, webhook timeout.

### Slice D (part 7) — admin_stats.go (454), trial_emails.go (554), healthcheck.go (46) — 31-08-26

**admin_stats.go: CLEAN.** adminAuth gate; parameterized filters; the B32 datetime-format fix verified in place; real-revenue merge has the double-count guard (r6). INFO notes (no change): full-table `FindRecordsByFilter` scans per request (admin-only; fine at current scale — use Count/aggregations if the tenant count grows); MRR/tier aggregates use float64 (display-only estimates on the Go side; `revenue_events` amounts remain int64 minor units per LSE-7); the IDR column converts historical months at the *current* FX rate; `countRecordsByFilter` fetches all rows instead of counting.

**healthcheck.go (46): CLEAN.** Standalone `//go:build ignore` binary used as the Docker HEALTHCHECK — 5s timeout, status check, proper exit codes. Nothing to fix.

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| LSE-23 | 🔴 HIGH (functional) ✅ FIXED 31-08-26 (`94b5abcc`) | trial_emails.go (both scanners) | **The trial-milestone and win-back email scanners never sent a single email in production**: they parsed PocketBase datetimes with `time.Parse(RFC3339, sub.GetString("starts_at"))`, but PocketBase datetime strings are `2006-01-02 15:04:05.000Z` (no "T" — see B32 in admin_stats) so the parse failed for EVERY record and both scanners silently skipped everything. | Fixed: `GetDateTime(...).Time()` accessor (same idiom as the working password_rotation scanner); zero datetimes skip with a log line. Regression test runs the scanner end-to-end against the STARTTLS test SMTP server and captures the day-7 email. |
| LSE-24 | 🟢 LOW ✅ FIXED 31-08-26 (`94b5abcc`) | trial_emails.go | Milestone matching used exact-day equality, so any scan skipped by downtime missed the email forever. | Fixed: `>=` catch-up semantics in both scanners — the `trial_email_log` idempotency records already exist precisely to prevent duplicates. Test proves a 9-day-old trial still gets exactly one day-7 email and a second scan sends nothing. |
| LSE-25 | 🟢 LOW ✅ FIXED 31-08-26 (`94b5abcc`) | trial_emails.go (sendTrialEmail) | The recipient was interpolated into RFC 5322 headers and RCPT TO without validation. | Fixed: `net/mail.ParseAddress` gate (rejects CRLF/control chars, multi-address strings) — defense in depth behind the PocketBase email field. |
| — | ℹ️ INFO | trial_emails.go (`getTrialUsageSummary`) | Returns placeholder usage stats ("several", "Rp ---") — documented stub, the upgrade CTA carries the email. The win-back day-offset `hashLogKey` int encoding can theoretically collide, irrelevant at 2 keys. | Documented; no action. |

**Artifact hygiene (31-08-26, `a5740604`):** `apps/license-server/coverage.out` (a Go coverage profile committed long ago in 2a145602) untracked and gitignored; `license-server.exe`/`coverage.html` were already ignored. Nothing in CI/scripts/Docker references it. Also verified this round: the pre-commit `cargo fmt` gate is working again (the blocking untracked `kds_lines.rs` is fixed and committed by the parallel session) — the last two commits ran the hooks without `--no-verify`.

**Slice D COMPLETE — and with it section 45 (apps/license-server, 32 production Go files, all read + stamped).** Files not named in earlier slices but covered: `healthcheck.go` (this part), `license_recover.go` (created + audited during LSE-11 phase A), `enterprise_trial.go` (part 5), `trial_emails.go` (this part), `revenue_events.go` (LSE-7 fix), `password_rotation.go` (via its test-side SMTP integration, part 6).

**Section 45 fix tally (FINAL):** HIGH fixed: LSE-1, LSE-17, LSE-23. MED fixed: LSE-5, LSE-6, LSE-8, LSE-9, LSE-11 (A+B), LSE-13, LSE-14, LSE-15, LSE-16, LSE-18, LSE-21, LSE-22. LOW fixed: LSE-19, LSE-20, LSE-25 (+B43/B45 lineage fixes). INFO-only files: api_key.go, status.go, expiry.go, health.go, admin_stats.go, healthcheck.go, helpers.go. **No open findings remain in section 45.**

---

## 46. Continuation phase — build/release/CI supply chain (`.github/workflows` + `scripts/` + Docker), opened 31-08-26

The last never-audited area: 11 workflows (~3.7k lines), ~100 dev/ops scripts, Docker assets. Risk-ranked slices: **A** release/deploy/security workflows (secrets + signing) · **B** ci.yml + e2e-pr.yml (untrusted input) · **C** nightly/android/ios/website/docker-* · **D** key & crypto scripts (`generate-license-keys`, `generate-tenant-keys`, `updater-crypto.mjs`, `generate-latest-json.mjs`, `extract-updater-seed.py`, `dev-code-sign.ps1`) · **E** DB/e2e scripts · **F** Dockerfiles/compose · **G** remaining scripts.

### Slice A+B — release.yml (500), deploy.yml (218), security.yml (124), ci.yml (1227), e2e-pr.yml (200) — 31-08-26

**Clean by design:** no `pull_request_target`/issue/comment triggers anywhere (fork PRs run secretless with a read-only token); every secret reaches `run:` through `env:` except the three findings below; fail-closed credential gates in deploy/website; draft→publish release flow with signed updater manifests, pubkey cross-check, SHA-256SUMS, and build-provenance attestation; Trivy CRITICAL/HIGH blocking scans; release.yml's `latest.json` assembly merges per-platform signatures with `--verify-pubkey`.

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| CICD-01 | 🟢 LOW ✅ FIXED 31-08-26 | 8 workflows | No top-level `permissions:` — jobs inherited the repo-default token (write-capable) unless they declared their own block. | Fixed: top-level `permissions: contents: read` on release/deploy/security/ci/e2e-pr/android/ios/website/nightly; per-job grants (`contents: write`, `packages: write`, `security-events: write`, `id-token+attestations`) unchanged and verified sufficient. |
| CICD-02 | 🟢 LOW ✅ FIXED 31-08-26 (`part A`) | e2e-pr.yml | `github.event.inputs.specs/headed/changed_only` interpolated **directly into the bash script** (CodeQL injection pattern; inputs are write-access-only so no secret exposure, but a foot-gun). | Fixed: env indirection (`INPUT_*`). |
| CICD-03 | 🟢 LOW ✅ FIXED 31-08-26 (`part A`) | ios.yml, release.yml, android.yml | Key material left on disk after import: `/tmp/cert.p12` + provisioning profile (ios), `$RUNNER_TEMP\cert.pfx` (release), decoded `oz-pos-release.keystore` + `keystore.properties` with plaintext passwords (android, never removed post-build). ios also interpolated secrets inline into `run:`. | Fixed: env indirection (ios) + `rm` after import/use (`if: always()` for the android cleanup); artifact upload paths were already correctly scoped (no keystore in artifacts). |
| CICD-04 | 🟡 MED ✅ FIXED 31-08-26 (`part A`) | scripts/verify-ci-docs-drift.py:72 | **The CI-docs-drift gate was broken on every run**: `DOCS = ROOT/"docs"/"ci-pipeline.md"` while the canonical doc lives at `docs/operations/ci-pipeline.md` (moved; the script's own docstring names the new path) — the `ci-docs-drift` CI job exited 2 unconditionally. | Fixed: path corrected; gate now runs and reports `0 drift item(s)` against the live workflows. |
| CICD-05 | 🟢 LOW ✅ FIXED 31-08-26 | all workflows | Third-party actions pinned by major/major.minor tag (`softprops/action-gh-release@v2`, `dorny/paths-filter@v3`, `aquasecurity/trivy-action@v0.36.0`, `docker/*@v3/v5`, `signpath/*@v2`, `actions/*`), not by commit SHA — the standard supply-chain hardening is SHA pinning for secret-touching jobs. | **Fixed:** all 210 `uses:` refs across the 11 workflows pinned to the exact commit SHA of the ref they used, resolved via the GitHub API and annotated with the original version comment (`@<sha> # v4`). Semantics preserved: the `dtolnay/rust-toolchain` `@stable`/`@nightly` version **branches** carry `default: stable`/`default: nightly` in their `action.yml`, and the pinned SHAs are those branch HEADs. Maintenance note: version bumps now require a deliberate SHA change — enable GitHub Dependabot (`package-ecosystem: "github-actions"`), which updates SHA pins automatically and preserves `# vX` comment annotations. |
| CICD-06 | ℹ️ INFO | release.yml (manifest assembly) | `find artifacts -name '*.exe' \| head -1` signs the FIRST matching installer — if a bundle ever emits two exes the manifest could sign the wrong one. The subsequent verify step re-finds the same file, so signature↔asset consistency holds. | Documented; consider explicit artifact paths per target if NSIS ever emits multiple exes. |

**Slice D (key/crypto scripts) — clean:** `generate-license-keys.sh` (OpenSSL genpkey CSPRNG, chmod 600, overwrite guard, private key git-ignored by `*.pem` rule), `generate-tenant-keys.sh` (`openssl rand -hex`, `/dev/urandom` fallback), `updater-crypto.mjs` (minisign format empirically verified against tauri-cli + plugin source; BLAKE2b-512 prehashed; keyid cross-check; global signature; self-tests + tamper rejection), `generate-latest-json.mjs` (key via env, never echoed), `extract-updater-seed.py` (offline rsign decryptor; prints the seed by design — operator tool), `dev-code-sign.ps1` (dev-only self-signed cert, documented as not for customers), `e2e-paddle-webhook.py` (webhook secret via env with CI test default). **No committed key material** — `git ls-files` clean, `.gitignore` covers `.env/`.key/.pem/.keystore/.p12/.pfx. **DB scripts clean:** `docker-entrypoint.sh` (gosu privilege drop with documented `su` shim), `backup-db.sh` (sqlite3 `.backup` + integrity check), `restore-db.sh` (integrity gate + confirmation), `reset-dev-pg.sh` (docker-exec, no SQL string building).

**Remaining:** Slice C deep-reads (nightly.yml 561, android/ios remainder), Slice E (e2e-otp-flow.mjs, run-e2e.mjs, dev-smtp-sink.py), Slice F (Dockerfiles/compose + verify-docker-*), Slice G (remaining ~70 scripts risk-ranked).

### Slices C/E/F/G — nightly.yml (566), website/android/ios tails, e2e+DB scripts, Dockerfiles/compose, installers, fuzz — 31-08-26

| ID | Sev | Location | Finding | Proposed solution |
|---|---|---|---|---|
| CICD-03b | 🟢 LOW ✅ FIXED 31-08-26 | nightly.yml `release-tablet-android` | Same keystore-hygiene gap as android.yml: decoded `oz-pos-release.keystore` + `keystore.properties` (plaintext passwords) left in the workspace after the build. | Fixed: same `if: always()` removal step added. |
| CICD-07 | ℹ️ INFO | install/install.sh + install/win/install.ps1 (public bootstrappers) | Fail-closed SHA-256 verification of installer + SHA256SUMS.txt from the same HTTPS release; self-checksum when run from disk; piped `curl \| bash` / `irm \| iex` mode skips self-verification and trusts the same release channel for both script and binary. | Documented by the scripts themselves and inherent to the GitHub-release model; the in-app updater path additionally carries minisign signatures. No action. |
| CICD-08 | ℹ️ INFO | release.yml artifact selection; install.sh `.app` pick | `find … \| head -1` / first-`.app` selection assumes exactly one matching artifact per platform (true today by construction). | Same as CICD-06 — explicit paths if bundles ever emit multiple artifacts. |
| CICD-09 | ℹ️ INFO | scripts/build-exe-release.ps1:121 | `Invoke-Expression $tauriCmd` — evaluates a constant two-branch string (no user-data interpolation), so safe but style-discouraged. | Optional: replace with `& cargo` argument arrays. |

**Verified clean in slices C/E/F/G:**
- **Workflows:** website.yml's deploy job (token-verify probe with user→account-scoped fallback, fail-fast gates, soft-fail docs portal, wrangler token via env); android/ios release-upload tails (60-min release poll, 3× retry with `--clobber`, artifact uploads scoped); docker-persistence.yml (PR-triggered verification only); docker-digest-drift.yml (its own `contents: read` + `issues: write` block — auto-opens a DOCKER-02 drift issue weekly); nightly.yml's flaky-report pipeline and full Go suite; benchmark artifacts 30-day retention.
- **Docker supply chain:** all 3 Dockerfiles digest-pin every base image (`DOCKER-02`, enforced by the weekly CI drift gate), run non-root (`DOCKER-01` — gosu/su-exec privilege drop with the documented `su` shim), keep runtimes minimal (ca-certificates+tzdata+su-exec; Go healthcheck binary instead of curl); all 5 compose files fail-closed on secrets (`${VAR:?}`), prod binds nothing internal (`ports: !override []`), e2e defaults are test-only and documented; `verify-docker-persistence.sh` runs the volume contract in CI.
- **e2e/DB scripts:** dev-smtp-sink binds 127.0.0.1 only; e2e-otp-flow talks to localhost CDP; run-e2e.mjs execSync with pinned image digests and no user-input interpolation; backup/restore DB scripts (integrity gate + confirm); generate-pg-migration.py emits `EXECUTE format('%I', …)`-parameterized identifiers; rls-cutover.sql clean.
- **Installers:** install.ps1 mirrors install.sh (TLS 1.2 enforcement, StrictMode, `finally` temp cleanup, msiexec argument-array quoting, advisory Authenticode with honest warning); uninstallers use scoped fixed paths and parse quoted UninstallStrings correctly.
- **fuzz/:** parse-only fuzz targets (cart/lua/manifest/money/ozpkg/percentage/sku); hfuzz overnight + triage scripts clean; no build artifacts tracked in git.
- **Repo-wide sweep:** no `curl | bash` of remote content beyond the documented bootstrappers, no `chmod 777`, no `Invoke-Expression` with interpolations, no eval of remote data in any of the ~100 scripts.
