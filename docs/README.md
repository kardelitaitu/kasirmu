...
- **Coverage — fixed.** The generator enumerates `docs/records/` as an input since
  ...
  generator (54 ADRs, 4 research, 17 phased, 11 audits, 14 scattered, 2 observability,
  13 records)` (measured 2026-09-23 — run the command for today's numbers, do not quote
  this sentence's); the record rows are the first table under `## Engineering Records
  (docs/records//)`. The older finding
...
- **Freshness enforcement — built 2026-09-23.** The bullet this replaces recorded the
  absence faithfully: `git grep -n generate-records-index -- .githooks .github
  scripts/check.sh` returned nothing (exit 1), because the wire was deliberately not
  pulled while two records were dirty in other sessions' working trees — a freshness
  gate that fires on content someone is mid-way through writing is an event that cries
  wolf — and it promised to land "in the same change as the first record that trips it,
  in a quiet tree." The 2026-09-23 documentation audit tripped it: `--check` was RED
  (148 generated vs 158 committed lines, ADR #60 missing) *and* the generator had a
  defect behind seven dead `snapshots/` rows, so the gate went in the same change as the
  repair: `check.sh` step `records index freshness`, `dev-ci.yml#ci-docs-drift` step
  `Records index freshness check`, gates.json `records-index` — all blocking from day
  one because the drift was fixed before the step landed (the inverse of the
  ci-docs-drift advisory→required arc, same reason: a known-red baseline gets disabled).
  Re-derive with `git grep -n generate-records-index -- scripts/check.sh
  .github/workflows/dev-ci.yml scripts/gates.json`.
...
>
> **Correction (23-09-26):** the 09-13-26 statement above — that the enforcement grep
> "still returns nothing, so the gate is unbuilt by decision and not by oversight" — is
> now false: the grep matches `scripts/check.sh`, `.github/workflows/dev-ci.yml` and
> `scripts/gates.json` as of today (see the *Freshness enforcement* bullet). The same
> pass repointed this page's five dead links (`guides/ARCHITECTURE.md`,
> `guides/EXTENDING.md`, `guides/QUICKSTART.md`, `plans/northflank-p1-p7-plan.md` →
