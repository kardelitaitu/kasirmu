...
### The records index is generated, and nothing regenerates it

`docs/records/README.md` opens with "Generated by `scripts/generate-records-index.mjs` —
do not edit by hand." That is accurate about the *editing*, and misleading about the
*generation*: no hook, no `check.sh` step and no CI job runs the script, so the file
silently falls behind every ADR added. Verified 08-09-26 — running it by hand produced a
diff that added **ADR #43 (cloud sync), #45, #46 and #47**, all of which had been merged
without appearing in the registry. Four decisions were invisible to anyone who trusts the
index, which is the entire audience for an index.

Refresh with `node scripts/generate-records-index.mjs` after adding or amending a record.
The script has no `--check`/`--dry-run` mode, so a CI freshness gate needs one added
first (run it to a temp copy and diff) — that is a code change and has not been made.
Two things the generator faithfully reproduces rather than fixes: the duplicate `#43`
emits two rows, and the audits section is empty by design because the script scans a
root `audit/` directory that no longer exists (its own comment records the
