...
## 39. Eight gates are `required` by policy with no `ci` block behind them -- `bundle-budget` was the seventh and left the class this week, and the remaining six are not six defects
...
- **RE-COUNTED 2026-09-19 by the ADR #55 lane -- the set is EIGHT, and it was already SEVEN before this lane added a row.** The guard reported `observed 8 / expected 6`, and the seventh member (`root-policy`) joined after this item was filed without a re-count, so the red predates this lane and is the ordinary drift this guard is built to surface: a row gained without CI coverage is a factual change to a census that a number in a title cannot absorb silently. The eight are `rust-clippy`, `root-policy`, `docker-dry-run`, `migration`, `e2e`, `perf-smoke`, `updater-signature`, `server-origins`. The new row is local-only **by design**, not by omission: `scripts/check-server-origins.mjs` parses the compiled origin list and every allowlist that must agree with it, needs `node` and nothing else, and would be a false claim in a `ci` block until someone adds it to `dev-ci.yml#static-gates`.
- **Re-derive, never re-read this number:** `python -c "import json;g=json.load(open('scripts/gates.json'))['gates'];print([x['id'] for x in g if x.get('status')=='required' and not x.get('ci')])"`.

- **RE-COUNTED 2026-09-20 -- the set is TEN, and both new members are the same local-only shape, not a lost `ci` key.** `scripts/check-font-claims.mjs` reported `observed 10 / expected 8`, and the two additions are `env-docs` (joined at `226812432` *ci(gates): require every read env var to be documented*) and `unified-routes` (joined at `eac292c91` *fix(unified): route the device-link endpoints to the licence server*). The ten are `rust-clippy`, `root-policy`, `docker-dry-run`, `migration`, `e2e`, `perf-smoke`, `updater-signature`, `server-origins`, `env-docs`, `unified-routes`. Measured, not inferred: `node -e "const g=require('./scripts/gates.json').gates;const r=g.filter(x=>x.status==='required'&&!x.ci);console.log(r.length);console.log(r.map(x=>x.id).join(','))"` → `10` and that exact list; both rows carry `status: "required"`, no `ci` key, and `runners: { "check.sh": [...] }` alone. The manifest is now **75 rows -- 58 required / 2 advisory / 15 retired** (was 71 / 54 / 1 / 16 when this item was filed), so the denominator moved with the numerator and the split is unchanged in kind. **The live value is the literal in `scripts/check-font-claims.mjs`, not this line** -- that row executes the count on every run and fails on drift, which is why this annotation is appended rather than substituted into the 2026-09-16 `6` and the 2026-09-19 `8`: those were correct at their own HEADs, and rewriting them would erase the drift this item exists to show.
