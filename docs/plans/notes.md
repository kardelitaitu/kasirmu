...
- **THE MECHANISM, in three lines of code.** `apps/desktop-client/src/state.rs` resolves the database as `app.path().app_data_dir()/oz-pos.db` with no environment override, and `tauri.conf.json` in **both** `0.0.39` and `0.0.38` carries `"identifier": "com.ozpos.app"`. Same identifier → same `app_data_dir` → the same file: `C:/Users/Dika/AppData/Roaming/com.ozpos.app/oz-pos.db`, measured at 1,956 KB with its mtime matching the panic timestamp to the second. **The tablet does not collide** -- its identifier is `com.ozpos.tablet`, which is why only `com.ozpos.app` exists under `%APPDATA%`. So this is desktop-against-desktop, across release branches.
