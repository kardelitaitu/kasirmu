...
## 36. `collectScriptFiles` in `themeTokenCompliance.test.ts` still swallows a failed `readdir`, the exact twin of the door `8cea958ed` closed for CSS — and `composedRuleIdenticalPair` carries a second copy
...
- **What this does NOT close.** Reachable is not enforced. `check:all` is a local command; no live workflow runs either budget leg (`git grep -l -i -e check-bundle -e 'bundle:check' -- '.github/workflows/*.yml'` → 0, re-run at this HEAD by `scripts/check-font-claims.mjs`), `scripts/check.sh` still has no step, and the manifest row still carries **no `ci` block**. Item 37's enforcement half is the surviving gap, and it now covers **both** artifacts -- which is the sentence to read before concluding the font payload is gated in CI, because it is not.
