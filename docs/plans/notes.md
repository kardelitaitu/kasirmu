...
- **Why release turns that seed into a panic, in one line of arithmetic.** The bypass is compiled out in release: `#[cfg(debug_assertions)]` at `crates/oz-core/src/license_verification.rs:391`, the line immediately above `if signature_base64 == "BOOTSTRAP_FREE"` inside `verify_license_signature`, so the value falls into a base64 decode that rejects `_` — and `_` is offset **9** of `BOOTSTRAP_FREE`, code point **95** (`python3 -c "print(ord('_'), 'BOOTSTRAP_FREE'[9])"` → `95 _`), which is where the `invalid symbol 95` of the forty-one panics comes from. Side finding, reported and not edited because that file is outside this page's fence: the comment at `:388`-`:390` still attributes the seed to "migration 061", and no file numbered 061 exists in `crates/oz-core/migrations/` — the seed is in the squashed init at `:1514`.
