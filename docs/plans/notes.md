...
## 36. `collectScriptFiles` in `themeTokenCompliance.test.ts` still swallows a failed `readdir`, the exact twin of the door `8cea958ed` closed for CSS
...
- **NOT THIS LANE'S TO FIX.** The collector belongs to the shadow/colour tail work, not to the font rules; a floor added here could turn red on that lane's account and would be committed under a message it did not choose. The patch is small and already proven next door, so it is offered rather than applied.
