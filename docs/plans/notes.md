...
- **What remains open, so this block is not read as a sweep: R4, R6, R8, R9 and R10 are untouched, and R1, R2, R5 are MOOT rather than closed** — a different state from done, and one that only a later R3 = yes or a decision to delete those rows can end. R4 (are the three non-goals binding), R6 (is `.ftl` the single content set), R8 (where the offline idempotency key lives), R9 (do dense grids stay on paging) and R10 (do the four Item-7 invariants become a graded contract) are all still waiting on the owner. **R8 may already be answered by evidence rather than by a ruling** — the Item-5 rows in `todo-review-type.md` were ticked on finding the key minted at enqueue, which is what R8's default said was absent; that needs checking before anyone rules, and this block does not claim it.
