...
> ✅ **What is live.** GitHub executes **FOUR** workflows (repro: `ls .github/workflows/*.yml`, measured 2026-09-24 → `android.yml`, `dev-ci.yml`, `release.yml`, `website.yml`): `release.yml` on `v*` tags, `dev-ci.yml` on `pull_request` + `push` to main + `workflow_dispatch`, `android.yml` on a `v*` tag or `workflow_dispatch` only (deliberately no PR trigger — `sed -n '38,42p' .github/workflows/android.yml`), and `website.yml` on `push` to main path-filtered to `website/**` + `prototypes/**`, plus `workflow_dispatch`. **This paragraph said TWO until 2026-09-23**, which was correct when written on 2026-09-14 and false from 2026-09-22, when `android.yml` was restored from `attic/` — a count that grows by one and is re-derived by nothing. **It said THREE until 2026-09-24**, when `website.yml` was restored from `attic/website.yml.bak`. Retired here is the earlier claim that `dev-ci.yml` was the only workflow GitHub executes. For `dev-ci.yml`, Every
...
> `ci.yml`, so once that file was retired it silently became "nothing is
> undocumented" and — previously — **four** live jobs (`website`,
> `cargo-nextest`, `northflank-deploy`, `static-gates`) went unlisted without a
> complaint. That is the history of the checker, not a count of today's jobs
> (`dev-ci.yml` now defines fourteen; it defined eleven before the 2026-09-24
> time-to-red split added `rust-fmt`, `go-gate` and `ipc-parity`).
>
> ⚠️ **What is history.** Rows whose Workflow column names `ci.yml`,
> `nightly.yml` or any other retired file document what *used* to
> gate a merge. (`website.yml` sat in this list until 2026-09-24, when it was restored;
> its rows below are present tense again.) The checker recognises them as history and does not count them as
...
|--------|--------------|----------|-------|
| `cargo-check` | ✅ Required | dev-ci.yml | `cargo check --workspace --all-targets --all-features`. Held the `cargo fmt --all -- --check` step until 2026-09-24, when fmt moved to `rust-fmt` so a formatting nit would stop waiting behind this job's checkout + toolchain + rust-cache + 10-package apt layer (44s measured). The apt layer, rust-cache, sccache and the `ui/dist*` stubs are all still needed here, because this job actually compiles |
| `rust-fmt` | ✅ Required | dev-ci.yml | step `cargo fmt --all -- --check` (was job `rust-fmt` in the retired `ci.yml`, then a step in `cargo-check`, and its own job again from 2026-09-24). Carries no apt layer, no rust-cache, no sccache and no dist stubs, so the verdict lands in ~20s instead of the ~50s it took behind `cargo-check`'s 44s of setup |
| `ipc-parity` | ✅ Required | dev-ci.yml | `python3 scripts/verify-ipc-parity.py` then `--self-test`. Both were steps in `static-gates` until 2026-09-24; the self-test measured 209s and 216s on two runs, 33% of that job. The real gate runs FIRST here, so a genuine parity violation still reports in ~30s while the liveness proof takes the slower slot. Path-gated on rust OR ui |
| `go-gate` | ✅ Required | dev-ci.yml | `gofmt -l` + `go vet` + `go test -short` on license-server. These three ran inside `static-gates` from 0.0.37 until 2026-09-24, when they moved here; gates.json now names `dev-ci.yml/go-gate`. **Path-gated on the new `go` router output** (`^apps/license-server/`) — before that output existed this path matched NO bucket, and Go was covered only because the steps sat in the ungated `static-gates` job. The original row said `ci.yml` until 08-09-26, which made live coverage look historical |
...
| `deploy` | ⬜ **Not a merge gate** (push to `main` + `workflow_dispatch` only) | website.yml | Website deploy: fail-fast Cloudflare token verify → `npm ci` → `npm run check` → `npm run build` → `check:links` → `check:seo` → `scripts/wrangler-deploy.sh` (SKIP_BUILD=1) plus its per-route verification. Path-filtered to `website/**` and `prototypes/**`; restored 2026-09-24 from `attic/website.yml.bak`. PRs are gated by `dev-ci.yml#website` instead, which is why this workflow carries no `pull_request` trigger |
...
| `android-build` | ⬜ **Not a merge gate** (tag + `workflow_dispatch` only) | android.yml | Builds the aarch64 APK and **fails if no APK is produced** — that assertion is the job's purpose, since `upload-artifact` only warns on an empty match. Also prints the APK's signing certificate, which is the value ADR #57 §2.1 fingerprints at runtime, so the on-device read has a recorded expectation. Deliberately has no PR trigger: a workflow nobody can run locally must not be able to block a merge. |
...
| `fuzz` | ❌ Runs nowhere | ci.yml | Fuzz targets exist under `tools/fuzz/`; gates.json marks the runner **retired**, so nothing executes them |
...
| Rust fmt | `rust-fmt` (dev-ci.yml) | Required | `check.sh` (cargo fmt) |
...
| Go (license-server) | `go-gate` | Required | `check.sh` (go fmt, go vet, go test (short)) |
| Website unit tests | `website` (dev-ci.yml) | Required | `check.sh` (website test) |
...
| `android.yml` | 🟢 **LIVE** (restored 2026-09-22, tag + dispatch only) | tag push (v*) + `workflow_dispatch` | Builds the Android APK for the tablet client (`apps/mobile-tauri`). Restored from `attic/` to close an ADR #57 gap: nothing in CI compiled the Android target, so a regression in the JNI signing-certificate read (§2.1) would have been caught by a person rather than a pipeline. **Not a merge gate** — no PR trigger — so a workflow that cannot be exercised locally cannot block anyone. The retired copy needed **corrections, not a rename**: paths (`apps/tablet-client` → `apps/mobile-tauri`, P11), JDK 17 → 21 (the guide pins 21), an explicit NDK install (its own header blamed the NDK toolchain for the stub), and no `tauri android init` (the scaffold is committed) or separate UI build (it moved into `beforeBuildCommand`). |
...
| `website.yml` | 🟢 **LIVE** (restored 2026-09-24 from `attic/website.yml.bak`, which stays as the historical copy) | `push` → main (path-filtered: `website/**`, `prototypes/**`) + `workflow_dispatch` | Builds the Astro site, runs its typecheck/tests/link/SEO gates, then deploys it with `scripts/wrangler-deploy.sh` and verifies one origin per declared route. One job, `deploy`; **no PR trigger** — `dev-ci.yml#website` already gates PRs. The retired copy's docs-portal half (Rust + mdBook into `public/docs-portal`) is deliberately not restored: production does not serve the portal (`/docs-portal/intro.html` → 404) |
...
>
> **Dated pointer correction (2026-09-24, time-to-red split).** Every `dev-ci.yml:NNN`
> pointer written above this line is now STALE, and they are deliberately not rewritten
> one by one. The split added a `concurrency:` block, a `rust-fmt` job, and two new jobs
> (`go-gate`, `ipc-parity`) whose comment blocks are themselves long, so the whole file
> shifted by roughly 150 lines — a mechanical rewrite of the numbers would be a claim of
> verification this pass did not perform on each one. Read job and step NAMES as the
> stable reference and re-derive a line number with `grep -n` before citing it; the two
> rows whose *content* changed, not just their pointers, are the `northflank-deploy` row
> (its `needs:` list gained three entries) and the `dev-ci.yml` inventory row (the total
> went eleven -> fourteen). Verified by re-reading the workflow: the job set, the deploy `needs:` list,
> the `go` route output and the `all()` helper, all of which `verify-ci-docs-drift.py` and
> `scripts/test-ci-routing.sh` now check mechanically.
