...
- [ ] **C1 [P0] Default to a real at-rest key instead of a public constant** (6.2, remediation P0-7)
  ...
  Done when: a build that persists credentials without OZ_MASTER_KEY either refuses to start or generates and keychains a key, and a test asserts the static fallback cannot be reached in a release build. First confirm with the owner whether any deployment sets OZ_MASTER_KEY (D1) - that decides urgency, not whether to fix it.
...
- [ ] **C2 [P0] Put an integrity gate on plugin loading** (6.4, P0-8)
  ...
  Done when: a modified .lua file is refused with a visible error and the previous plugin set keeps running; required_permissions become an operator grant rather than a self-declaration; allow_network / allow_filesystem / allow_http are either enforced or deleted.
...
- [ ] **C7 [P0] Turn tenant isolation on for real - as a role change, not a schema edit** (7.1, P0-6)
  ...
  Sequence: create the oz_app role, grant DML, switch the connection, verify, and only then consider FORCE ROW LEVEL SECURITY as a follow-up. Keep the webhook BYPASSRLS path and the /status tolerance. Never add FORCE to the generated migration while the app connects as the table owner - every query would return zero rows on deploy.
...
- [ ] **C8 [P0] Make recovery real: restore in the app, a backup that survives, verification on both sides** (14.1, P0-9)
  ...
  Done when: a bridge-level restore_roundtrip test backs up through the same command the UI calls, corrupts the live database, restores from the app path, and asserts check_integrity() is Ok with a known sale reading back; a second case asserts a corrupt backup is refused and the live file is left byte-identical. Backups write to a temporary name and rename over the previous only on success; at least two generations are kept; the app either quiesces or refuses a restore while another process holds the database.
...
- [ ] **C9 [P0] Decide what qris-core is, and make the manifest say it** (14.4, P0-10)
  ...
  Done when: either it is proprietary like everything else (publish = false, license.workspace = true, a deny.toml clarification entry) or it is genuinely dual-licensed (LICENSE-MIT and LICENSE-APACHE committed, a real repository and authors, the deny.toml entry). cargo deny must no longer be blind to the difference. Owner decision D4.
...
- [ ] **C26 Decide the architecture boundary before 2026-11-06** (10.2, 10.3) - re-tier the checker with a named rule for the seven type-only re-export edges and an ADR, or move the model types back down; either way add a checker test that a bumped expiry without a new reason fails. Owner decision D3.
- [ ] **C27 Reconcile the container's proxy with the licence server's routes** (12.1) - apps/unified/Caddyfile:44-72 misses pairing/* and midtrans/*, so five registered routes 404 in the shipped image; add a test that compares the route table to the carve-out list so this cannot drift again.
...
- [ ] **C29 Declared-but-inert surface** - decide per item (13): file/rotation log sinks, the rate-sync daemon, the scale driver that is never registered, the EDC stubs registered on a money path, the modules' test-only services, kasirmu-media (one-line workspace-exclude first, then delete), the empty plugin IPC surface.
