...
The `.githooks/pre-commit` hook runs **ten steps** automatically before every commit, in this order (~5–7s total on a typical commit; the i18n gate alone is ~4s). Step 9 is conditional and adds ~21s **only** when the commit stages `ui/src` TypeScript — measured end to end at 28.7s for a UI commit versus 7.1s for a docs-only one. Step 10 fires only when the commit stages a `.ftl` file:
...
9. **`UI typecheck`** — when the commit stages `ui/src/*.ts` or `ui/src/*.tsx`: `npm run typecheck` (~21s), hard fail. Added in **0.0.37** after ten commits landed with HEAD failing `tsc --noEmit`: Vitest injects `describe/it/expect/beforeEach/vi` at runtime, so a test file that forgets to import one **passes `npm run test`** and only fails `tsc` — and nothing ran `tsc` before commit. `check.sh` always had it, but `check.sh` is opt-in. `OZPOS_SKIP_TYPECHECK=1` skips this step alone (prefer it to `--no-verify`, which skips all ten). Warns instead of failing when `ui/node_modules` is absent, so the skip is announced rather than invisible. `scripts/test-typecheck-gate.sh` extracts the live step from the hook and drives it against a throwaway repo with a stubbed `npm`, because the step only fires on UI commits and would otherwise go unexercised long enough to be indistinguishable from deleted.
10. **`FTL orphan lint`** — when the commit stages a `.ftl` file: `scripts/verify-ftl-orphans.py --staged-only`, hard fail. The mirror of step 4, which only asks whether code names a key no bundle defines. Nothing ever asked whether a key has any code that reads it, and the debt is real: `topology-shortcuts-*` (18 keys whose feature was removed, per `popoverSurfaceCompliance.test.tsx:54`) and ~23 `warehouse-*` keys with zero references anywhere. **Staged-scoped deliberately** — a whole-tree blocker is unusable because dynamic composition (`analytics-month-${m}`) makes an entire family live from one template hit, leaving 93 honest candidates of which an unknown fraction are detection gaps rather than debt. So it holds a commit to two things a commit can answer: a key you *add* must be referenced, and a reference you *delete* must not strand a key. `--census` reports the tree picture informationally (in CI, non-blocking); `--self-test` exercises all four parser directions and blocks in CI, because a gate whose diff-parsers silently break checks nothing.

> ✅ **All ten steps now have a CI backstop.** Steps 6 and 7 (`verify-migration-column-types.py`, `generate-pg-migration.py --check`) lived in `ci.yml`, were retired to `.bak` by `23c96330`, and were restored into `dev-ci.yml#static-gates` in **0.0.37** — until then the opt-in hook was their only guard, and they had **no `gates.json` record at all**, so the drift checker could not even report them as unenforced. CI runs the column-type check without `--staged-only`, so it scans all 28 migration files rather than only the ones a commit happened to touch. Both are pure text comparison (~1.1s together): no Docker, no psycopg, no sqlite handle — there was never a build-cost reason to leave them out. Step 8 (Go) has been CI-backed since 0.0.36: `dev-ci.yml#static-gates` runs `gofmt -l`, `go vet` and `go test -short` on `apps/license-server`. The hook's comments used to cite `.github/workflows/ci.yml` in four places (the i18n, migration-column-types, pg-schema-drift and Go steps) — that workflow is retired (`.bak`), so the coverage was real but every pointer was wrong. **All four now name `dev-ci.yml`, and `verify-ci-docs-drift.py` polices it:** any `.github/workflows/<name>.yml` mentioned in the hook must be a live workflow file, so a future retirement that leaves the prose behind fails the gate instead of quietly inverting the claim. Note the CI gate is `gofmt -l` (report-only, fails on any unformatted file) while the hook runs `gofmt -w` and re-stages, so a commit made without `core.hooksPath` can be unformatted and CI will reject it rather than fix it. **Step 9's CI backstop is `dev-ci.yml#ui-test`, which runs `npm run typecheck` — but that job only runs on a PR to `main`, and `dev-ci.yml` has no `push` trigger, so on a long-lived release branch the hook is currently the only thing typechecking at all.** Step 10's backstop is `dev-ci.yml#i18n`, which cannot reproduce the staged check (CI has no index to compare against) so it runs `--self-test` blocking and `--census` informational instead — meaning **the orphan gate's enforcement is local-only, exactly like step 9's was before `ui-test` existed.** Recorded rather than glossed: on this branch the hook is the only thing that will ever reject an orphaned key.
...
- **⚠️ `git commit --amend` needs an explicit pathspec too, and verifying the index first is NOT sufficient.** `--amend` commits the whole index, so it inherits every file another agent has staged since your last look. This was hit while *following* the rule above: `git diff --cached --name-only` printed only my file, and four seconds later `git commit --amend -m ...` produced a 5-file commit containing someone else's `.circleci/config.yml`, `scripts/compose-circleci.py`, and two generated workflow files. The check was true when it ran and false by the time it mattered — with a concurrent agent the index is a racing value, so a pre-commit inspection of it proves nothing. Recovery is `git reset --mixed HEAD~1` (moves the commit, leaves the working tree and everyone's staging alone), then re-commit **with** a pathspec. Prefer not amending on a shared branch at all: a pathspec-limited commit is immune to the race, an amend is not.
