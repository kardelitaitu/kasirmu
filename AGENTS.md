...
The `.githooks/pre-commit` hook runs **eight steps** automatically before every commit, in this order (< 1s total on a typical commit; path-scoped to prevent unneeded work across multi-agent concurrency):
1. **`cargo fmt`** — auto-formats Rust and re-stages what it changed; runs only when `.rs` files are staged (bypasses 40+ crate scan on docs/UI commits).
...
4. **`FTL dedupe dry-run`** — runs `scripts/dedupe-ftl.py --dry-run` when `.ftl` files are staged to detect duplicate Fluent keys before push.
