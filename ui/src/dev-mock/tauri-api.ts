...
const entryHandlers: Record<string, MockHandler> = {
  ...
  'pick_logo_file': () => null,
  // Catalog mocks — products, variants, categories, currency and tax —
  // now live in `handlers/catalog.ts` (agent 2, phase 2.1).
  //
  // `adjust_stock` below deliberately stays: it is an inventory mutation,
  // which is phase 2.2's `handlers/inventory.ts` fence.
