...
describe('Tooltip', () => {
  ...
  // ── disabled prop (double-tooltip regression) ──────
  //
  // The settings sidebar renders nav tooltips via portal, so the old
  // CSS suppression (`.settings-sidebar:not(.collapsed) .tooltip-content
  // { display:none }`) could never reach the bubbles — hovering a nav
  // item in the EXPANDED sidebar popped a bubble repeating its visible
  // label ("double tooltip"), and the pin button's native title stacked
  // with the React bubble. Suppression is now component-level: disabled
  // tooltips render the trigger only.

  describe('disabled prop', () => {
    it('renders no tooltip bubble at all when disabled', () => {
      renderTooltip({ disabled: true, portal: true });
      expect(getTooltipContent()).toBeNull();
    });

    it('does not show on hover or focus when disabled', () => {
      ...
      // Nothing rendered to show — wrapper hosts the trigger only.
      expect(getTooltipContent()).toBeNull();
      expect(screen.getByTestId('trigger')).not.toHaveAttribute('aria-describedby');
    });

    it('does not add portal nodes to document.body when disabled', () => {
      renderTooltip({ disabled: true, portal: true });
      const bodyBubbles = document.body.querySelectorAll(':scope > .tooltip-content');
      expect(bodyBubbles.length).toBe(0);
    });

    it('renders children intact (trigger keeps its own semantics)', () => {
      renderTooltip({ disabled: true });
      expect(screen.getByTestId('trigger')).toBeInTheDocument();
      expect(screen.getByTestId('trigger').textContent).toBe('Hover me');
    });

    it('shows normally again once disabled is lifted (collapse toggle)', () => {
      const { rerender } = renderTooltip({ disabled: true });
      const wrapper = getTooltipWrapper()!;
      ...
      // Sidebar collapses → disabled flips false → tooltip works again.
      // remount semantics: re-render as a fresh enabled instance.
      rerender(
