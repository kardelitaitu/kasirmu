...
`npm run dev` is what `cargo tauri dev` (from `apps/desktop-tauri/`) launches.
...
> Both match the `devUrl` keys in `apps/desktop-tauri/tauri.conf.json:8` and
