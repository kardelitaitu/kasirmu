...
// README.md is excluded the way the decisions scan excludes its own README, and
// the filenames are sorted so a non-POSIX readdir order cannot produce a phantom
// diff in a future commit. Front matter overrides title/status/area.
...
console.log(`Wrote ${OUT} (${numbered.length} ADRs, ${research.length} research, ${phases.length} phased, ${audits.length} audits, ${scattered.length} scattered, ${observability.length} observability, ${records.length} records)`);
