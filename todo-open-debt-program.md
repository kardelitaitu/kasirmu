...
- [ ] **Then share the DTOs, one domain per commit**, following the T2/T3 pattern: bridge owns the type, the tablet re-exports it, a back-compat alias carries the old spelling, and a wire-deserialization test pins the frontend's actual payload.
  - **PENDING as of 2026-09-15 — left open on purpose by the lane that closed `:154`, and the reason is attribution, not absence of work.** The sharing step is running RIGHT NOW as three separate lanes against `apps/tablet-client/src/commands/sync.rs`, `apps/tablet-client/src/commands/setup.rs` and `ui/src/api/hardware.ts`. At the hour this line was written the first and the third were ` M` in this checkout and **no commit SHA exists for any of the three**, so a tick here would credit a green to code that is mid-edit. The shape this box is earned by is T2/T3, not the finding: bridge owns the type, the tablet re-exports it, a back-compat alias carries the old spelling, and a wire-deserialization test pins the payload the UI actually sends — the last clause is the one nothing in this file has yet observed running, and it is the contract-lock that separates this row from the audit above it. (Pointer hygiene: this append shifts every row at or after the old `:156` by **+7** — that row is `:163` at the commit that landed the audit note above — so re-take any tail pointer against the row text, not the number, per this file's own counting section.) Re-read all three fences before ticking, and note that `cargo test -p oz-pos-tablet` (this phase's acceptance) was not run by the audit pass either.
